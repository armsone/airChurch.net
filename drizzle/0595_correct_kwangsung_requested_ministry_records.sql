-- User-approved removal of 17 affiliations with 거룩한빛광성교회.
...
WHERE church_name='거룩한빛광성교회'
  AND pastor_id IN (
    ...
    OR (public_id=0 AND name='정성진')
  );

-- 엄유현: remove obsolete 교육전도사 only; keep current 준전임전도사.
DELETE FROM pastor_church_roles
WHERE pastor_id IN (SELECT id FROM pastor_people WHERE public_id=15449 AND name='엄유현')
  AND church_name='거룩한빛광성교회'
  AND role_title='교육전도사';
...
WHERE church_id IN (SELECT id FROM churches WHERE public_id=1 AND name='거룩한빛광성교회')
  AND (name IN ('김환', '김환 목사', '문상원', '문상원 목사', '박순심', '박순심 목사', '윤화평', '윤화평 목사', '한요한', '한요한 목사', '홍요한', '홍요한 목사', '신연섭', '신연섭 목사', '윤성로', '윤성로 목사', '응웬반떼', '응웬반떼 목사', '이대설', '이대설 목사', '이재성', '이재성 목사', '쯩티탄림', '쯩티탄림 목사', '황경희', '황경희 목사', '류후춘', '류후춘 목사', '전춘미', '전춘미 목사', '천영철', '천영철 목사', '정성진', '정성진 목사') OR (name='엄유현' AND role_title='교육전도사'));
