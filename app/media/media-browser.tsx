...
type MediaItem={churchId?:number;youtubeId:string;title:string;thumbnailUrl:string;publishedAt:string;church:string;pastor:string;region:string;denomination:string};
...
export default function MediaBrowser({kind}:{kind:"sermon"|"short"}){
  ...
  return <>
    ...
    {kind==="sermon"&&<div className="media-topics" aria-label="말씀 주제 검색">{["기도","위로","믿음","가정","감사","용서","사랑","소망"].map(topic=><button type="button" key={topic} aria-pressed={query===topic} onClick={()=>search(topic)}>{topic}</button>)}<small>제목·교회 정보에 포함된 단어로 찾습니다.</small></div>}
