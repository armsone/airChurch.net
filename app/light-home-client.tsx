...
export default function LightHome(){
  ...
  return <main className="light-home simple-home" id="top">
    ...
    <section className="simple-home-grid" aria-label="airChurch 첫 화면 안내">
      ...
      <a className="simple-home-card saved" href="/portal#continue-title"><span>나의 모음</span><h2>{savedItems.length?`${savedItems.length}개를 모아 두었어요`:"마음에 남은 것을 모아 두세요"}</h2><p>{savedItems.length?savedItems.slice(0,2).map((item)=>item.title).join(" · "):"로그인 없이 이 브라우저에만 보관합니다."}</p><b>포털에서 보기 →</b></a>
      <a className="simple-home-card season" href="/portal#season-title"><span>교회력으로 걷는 오늘</span><h2>{season.name}</h2><p>{season.copy}</p><b>오늘의 말씀 보기 →</b></a>
      <a className="simple-home-card search" href="/portal#topic-discovery"><span>마음에서 시작하는 검색</span><h2>이름보다 마음을 먼저 떠올려도 괜찮아요</h2><p>말씀, 교회, 목사님을 한 포털 안에서 찾습니다.</p><b>검색 시작하기 →</b></a>
      <a className="simple-home-card statement" href="/portal#vision"><span>airChurch가 지키는 한 문장</span><h2>말씀과 교회를 정직하게 연결합니다</h2><p>발견은 airChurch에서, 소속과 돌봄은 건강한 지역교회와 함께합니다.</p><b>소개 보기 →</b></a>
      <a className="simple-home-card safety" href="/portal#principles"><span>건강한 신앙 생태계</span><h2>열린 문에는 분명한 기준이 필요합니다</h2><p>공식 출처를 확인하고, 문제가 제기되면 다시 살핍니다.</p><b>운영 기준 보기 →</b></a>
