...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    const loaders: Record<string, () => void> = {
      ...
      "church-directory": ()=>loadItems("/api/churches?catalog=58").then((data)=>{
        if(!alive) return;
  ...
  return (
    <main>
      ...
      <section className="church-directory-section" id="church-directory">
        ...
        {churchLoading?<div className="church-directory-grid"><LoadingCards count={6} /></div>:<div className="church-directory-grid">{visibleChurches.map((church)=><article key={church.id}><span>{church.region}</span><div className="church-name-row"><h3>{church.name}</h3>{church.channelImageUrl&&<img src={church.channelImageUrl} alt="" loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={(event)=>{event.currentTarget.hidden=true}} />}</div><p>{church.pastor}</p><small>{church.denomination}</small><div className="church-directory-links">{church.homepageUrl&&<a href={church.homepageUrl} target="_blank" rel="noreferrer" title={`${church.name} 공식 홈페이지`} aria-label={`${church.name} 공식 홈페이지 열기`}><span className="directory-icon website-icon" aria-hidden="true" /></a>}{church.youtubeChannelId&&<a className="youtube-link" href={`https://www.youtube.com/channel/${church.youtubeChannelId}`} target="_blank" rel="noreferrer" title={`${church.name} 공식 YouTube`} aria-label={`${church.name} 공식 YouTube 열기`}><span className="directory-icon youtube-icon" aria-hidden="true" /></a>}</div></article>)}{!filteredChurches.length&&<div className="empty">조건에 맞는 등록 교회가 없습니다. 아래에서 추천해 주세요.</div>}</div>}
      ...
      <footer id="page-bottom">
        <HomeReloadLink className="brand footer-brand"><span className="brand-mark" aria-hidden="true" /><span>airchurch</span></HomeReloadLink><p>airchurch.net · goodshare.net · linechurch.net<br />말씀과 선한 영향력을 잇는 하나의 공동체</p><div><a href="#church-directory">등록교회</a><a href="#principles">운영원칙</a><a href="#vision">비전</a><a href="#community">문의</a><a href="/admin">관리자</a><a href="/review">목사님</a></div>
      </footer>
    </main>
  );
}
