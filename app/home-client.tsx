...
type PastorItem = { person_id:number|null; role_id:number|null; church_id:number|null; minister_id:number|null; name:string; role_title:string; role_titles:string; role_status:string; church_name:string|null; region:string|null; denomination:string|null; photo_url:string|null; merged_count:number };
  ...
  return (
    <main id="top"><SkipLink/>
      ...
      <section className="hero" id="primary-content" tabIndex={-1}>
        ...
        <form className="search" role="search" onSubmit={(event)=>{event.preventDefault();const term=query.trim(),normalized=normalizeSearchValue(term),params=new URLSearchParams();if(normalized){params.set("q",term);const next=[term,...recentSearches.filter((item)=>normalizeSearchValue(item)!==normalized)].slice(0,5);setRecentSearches(next);try{writeRecentSearches(next);}catch{/* 저장이 제한된 브라우저에서도 검색은 계속합니다. */}}if(region!=="전체")params.set("region",region);if(denomination!=="전체 교단")params.set("denomination",denomination);window.location.assign(`/search${params.size?`?${params}`:""}`);}}>
      ...
      <section className="church-directory-section pastor-home-directory" id="pastor-directory">
        ...
        {pastorLoading?<div className="pastor-directory-grid" id="pastor-directory-grid"><LoadingCards count={8}/></div>:<div className="pastor-directory-grid" id="pastor-directory-grid">{pastorItems.slice(0,pastorVisibleCount).map((pastor)=>{
          const name=pastor.name.replace(/\s*목사(?:님)?$/u,""),href=pastor.person_id?`/pastors/p/${pastor.person_id}`:`/pastors/${pastor.church_id}${pastor.minister_id?`?minister=${pastor.minister_id}`:""}`,photo=pastor.photo_url||"/pastor-silhouette-soft.png",roles=(pastor.role_titles||pastor.role_title).split(",").filter(Boolean),savedId=`pastor:${pastor.person_id??`${pastor.church_id}-${pastor.minister_id??"primary"}`}`,manageable=isAdmin&&Boolean(pastor.person_id);
          const toggleAdmin=(card:HTMLElement)=>{const details=card.querySelector<HTMLDetailsElement>(".admin-pastor-details");if(details)details.open=!details.open;};
          return <article className={`pastor-home-card${manageable?" is-admin-card":""}`} key={savedId} role={manageable?"button":undefined} tabIndex={manageable?0:undefined} aria-label={manageable?`${name} 목회자 관리 열기`:undefined} onClick={manageable?(event)=>{if((event.target as HTMLElement).closest("button,input,select,textarea,label,form,summary,.admin-pastor-details"))return;if((event.target as HTMLElement).closest("a"))event.preventDefault();toggleAdmin(event.currentTarget);}:undefined} onKeyDown={manageable?(event)=>{if(event.target!==event.currentTarget||!["Enter"," "].includes(event.key))return;event.preventDefault();toggleAdmin(event.currentTarget);}:undefined}><a href={href} className="pastor-directory-card"><span className={`pastor-directory-photo${pastor.photo_url?" has-photo":" is-placeholder"}`}><img src={photo} alt={pastor.photo_url?`${name} 목회자`:""} width={92} height={116} loading="lazy" decoding="async" referrerPolicy="no-referrer"/></span><span className="pastor-directory-copy"><span className="pastor-directory-status">{pastor.role_status==="former"?"사역 이력":"현재 사역"}</span><strong>{name}</strong><span className="pastor-directory-roles">{roles.map((role)=><b key={role}>{role}</b>)}</span>{pastor.church_name&&<p>{pastor.church_name}</p>}{(pastor.region||pastor.denomination)&&<small>{[pastor.region,pastor.denomination].filter(Boolean).join(" · ")}</small>}<em>{manageable?"카드를 눌러 정보 관리":"목회 기록과 응원글 보기 →"}</em></span></a><button className={`church-save pastor-home-save${isSaved(savedId)?" is-saved":""}`} type="button" onClick={()=>toggleSaved({id:savedId,kind:"pastor",title:name,subtitle:[pastor.church_name,pastor.role_title].filter(Boolean).join(" · "),url:href})} aria-label={`${name} 목회자 ${isSaved(savedId)?"찜에서 빼기":"찜하기"}`}>{isSaved(savedId)?"♥":"♡"}</button>{manageable&&pastor.person_id&&<Suspense fallback={null}><PastorControls id={pastor.person_id} roleId={pastor.role_id} name={pastor.name} roleTitle={pastor.role_title} churchName={pastor.church_name} region={pastor.region} denomination={pastor.denomination} roleStatus={pastor.role_status} status="approved"/></Suspense>}</article>;
