...
export function hasSavedItemNewSermon(item:SavedItem,sermons:{church:string;pastor:string;publishedAt?:string}[]){
  ...
  const pastorName=normalized(item.pastorName??item.title.replace(/\s*(?:담임|위임|대표|협동|원로|은퇴)?목사(?:님)?$/u,"")),churchNames=(item.churchNames!==undefined?item.churchNames:[item.subtitle]).map(normalized);
  return sermons.some((sermon)=>{const same=item.kind==="pastor"?normalized(sermon.pastor).includes(pastorName)&&(churchNames.length===0||churchNames.includes(normalized(sermon.church))):normalized(sermon.church)===normalized(item.title);return same&&!!sermon.publishedAt&&Date.parse(sermon.publishedAt)>savedAt;});
}
