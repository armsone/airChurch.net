...
const jumps=[["오늘의 성경","#daily-scripture"],["쇼츠","#shorts"],["교회 찾기","#church-directory"],["목회자 찾기","#pastor-directory"],["착한나눔","#goodshare"],["달란트","#talent"],["소개·원칙","#vision"]];
const normalize=(value:string)=>value.replace(/\s/g,"").replace(/목사(?:님)?$/u,"").toLocaleLowerCase("ko-KR");
const panelThemes:Record<string,{icon:string;tone:string}>={"말씀":{icon:"📖",tone:"word"},"찬양":{icon:"🎵",tone:"praise"},"뉴스":{icon:"📰",tone:"news"},"교회":{icon:"⛪",tone:"church"},"목회자":{icon:"👤",tone:"pastor"},"행사":{icon:"📅",tone:"event"},"익명광장":{icon:"💬",tone:"community"}};
...
export default function PortalToday({news,sermons,saved,now,newsLoading,sermonLoading,refresh,rankings,posts,region,onRankingMore}:Props){
 ...
 return <section className="portal-today" aria-labelledby="portal-today-title">
  ...
  <div className="portal-overview portal-eight">
   ...
   <Panel title="익명광장" href="#community"><p className="portal-caption">이야기 · 기도 · 달란트 나눔</p><ul className="portal-picks">{posts.slice(0,4).map(post=><li key={post.id}><a href={`#community-post-${post.id}`}><strong>{post.content.slice(0,75)}{post.content.length>75?"…":""}</strong><small>{post.category} · {post.nickname}</small></a></li>)}</ul>{!posts.length&&<p className="portal-empty">아직 공개된 이야기가 없습니다. 광장에서 첫 마음을 나눠보세요.</p>}</Panel>
  </div>
