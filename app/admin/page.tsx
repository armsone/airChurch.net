...
type ReviewerOpinionRow = { church_id:number; reviewer_name:string; status:string; note:string|null; reviewed_at:string };
...
export default async function AdminPage() {
  ...
  const reviewerOpinions=await db.prepare("SELECT r.church_id,COALESCE(a.name,'관리자') AS reviewer_name,r.status,r.note,r.reviewed_at FROM reviewer_church_reviews r LEFT JOIN reviewer_accounts a ON a.id=r.reviewer_id ORDER BY r.reviewed_at DESC LIMIT 1000").all<ReviewerOpinionRow>();
