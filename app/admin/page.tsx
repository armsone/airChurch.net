...
export default async function AdminPage() {
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-management-grid">
      <article className="admin-panel" id="church-management"><div className="admin-panel-title"><div><small>CHURCH MANAGEMENT</small><h2>공개 교회 관리</h2><p>관심도가 높은 교회는 목록과 최신 말씀에서 더 자주 소개됩니다.</p></div><span>{publicChurchRows.length}곳</span></div><div className="admin-manage-list">{publicChurchRows.length ? publicChurchRows.map((church) => <article key={church.id}><div className="admin-record-heading"><strong>{church.name}</strong><div className="admin-record-status"><span className={`priority-${church.priority_weight}`}>{church.priority_weight === 3 ? "♥ 매우 높음" : church.priority_weight === 2 ? "♥ 높음" : "기본"}</span><span className={`status-${church.review_status}`}>{statusLabel(church.review_status)}</span></div></div><ChurchControls id={church.id} name={church.name} pastor={church.pastor} region={church.region} denomination={church.denomination} status={church.review_status} holdReason={church.hold_reason} holdNote={church.hold_note} heldAt={church.held_at} priorityWeight={church.priority_weight} /></article>) : <p className="admin-empty">공개 중인 교회가 없습니다.</p>}</div></article>
      <article className="admin-panel" id="church-hold"><div className="admin-panel-title"><div><small>CHURCH HOLD</small><h2>보류 교회</h2><p>보류 사유와 메모는 관리자에게만 표시됩니다.</p></div><span>{heldChurchRows.length}곳</span></div><div className="admin-manage-list">{heldChurchRows.length ? heldChurchRows.map((church) => <article key={church.id}><div className="admin-record-heading"><strong>{church.name}</strong><span className={`status-${church.review_status}`}>{statusLabel(church.review_status)}</span></div><ChurchControls id={church.id} name={church.name} pastor={church.pastor} region={church.region} denomination={church.denomination} status={church.review_status} holdReason={church.hold_reason} holdNote={church.hold_note} heldAt={church.held_at} priorityWeight={church.priority_weight} /></article>) : <p className="admin-empty">보류된 교회가 없습니다.</p>}</div></article>
