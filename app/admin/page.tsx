...
export default async function AdminPage() {
  ...
  const [today, week, month, active, hourly, daily, monthly, churches, recommendations, churchRows, postRows, talentRows, recommendationRows, contactRows] = await Promise.all([
    ...
    db.prepare("SELECT id,name,pastor,region,denomination,review_status,hold_reason,hold_note,held_at,priority_weight,homepage_url,youtube_channel_id,channel_image_url FROM churches WHERE review_status IN ('approved','removed') ORDER BY CASE review_status WHEN 'approved' THEN 0 ELSE 1 END,priority_weight DESC,name LIMIT 2000").all<ChurchRow>(),
  ...
  const publicPreviewIds=randomChurchIds(publicChurchRows,20);
  const heldPreviewIds=randomChurchIds(heldChurchRows,10);
