...
import { ChurchControls, ReviewControls, ReviewerAccountControls } from "./admin-controls";
...
type ChangeRequestRow={id:number;request_type:string;reason:string;status:string;admin_note:string|null;created_at:string;reviewer_name:string;church_name:string;church_pastor:string;church_region:string;church_denomination:string;proposed_name:string|null;proposed_pastor:string|null;proposed_region:string|null;proposed_denomination:string|null};
type ConcernOpinionRow={id:number;church_id:number;reviewer_name:string;note:string|null;reviewed_at:string;admin_resolution:string|null;admin_note:string|null;church_name:string;church_pastor:string;church_region:string;church_denomination:string;church_status:string;hold_reason:string|null;hold_note:string|null};
...
export default async function AdminPage() {
  ...
  const [today, week, month, active, hourly, daily, monthly, paths, churches, recommendations, churchRows, postRows, talentRows, recommendationRows] = await Promise.all([
    ...
    db.prepare("SELECT id,name,pastor,region,denomination,review_status,hold_reason,hold_note,held_at,priority_weight FROM churches WHERE review_status IN ('approved','removed') ORDER BY CASE review_status WHEN 'approved' THEN 0 ELSE 1 END,priority_weight DESC,name LIMIT 2000").all<ChurchRow>(),
  ...
  const [reviewerRows,changeRequestRows,pendingConcernRows]=await Promise.all([
    ...
    db.prepare("SELECT r.id,r.request_type,r.reason,r.status,r.admin_note,r.created_at,COALESCE(a.name,'삭제된 계정') AS reviewer_name,c.name AS church_name,c.pastor AS church_pastor,c.region AS church_region,c.denomination AS church_denomination,r.proposed_name,r.proposed_pastor,r.proposed_region,r.proposed_denomination FROM church_change_requests r JOIN churches c ON c.id=r.church_id LEFT JOIN reviewer_accounts a ON a.id=r.reviewer_id WHERE r.status IN ('pending','deferred') ORDER BY CASE r.status WHEN 'pending' THEN 0 ELSE 1 END,r.created_at ASC LIMIT 1000").all<ChangeRequestRow>(),
