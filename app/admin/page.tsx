...
export default async function AdminPage() {
  ...
  if (!allowedEmail || user.email.toLowerCase() !== allowedEmail) {
    return <main className="admin-shell"><section className="admin-denied"><span className="brand-mark">A</span><h1>관리자 전용 페이지입니다</h1><p>허용된 관리자 계정으로 다시 로그인해 주세요.</p><a href={chatGPTSignOutPath("/admin")}>다른 계정으로 로그인</a></section></main>;
  }

  const db = database();
  await Promise.all([ensureAnalyticsTables(db), ensureCommunityTables(db), ensureSermonTables(db)]);
  const [today, week, month, daily, paths, sermons, churches, posts, talents] = await Promise.all([
  ...
  return <main className="admin-shell">
    <header className="admin-header"><Link className="brand" href="/"><span className="brand-mark">A</span><span>airchurch</span></Link><div><span>{user.displayName}</span><a href={chatGPTSignOutPath("/")}>로그아웃</a></div></header>
    ...
    <section className="admin-operations"><article><small>공개 교회</small><strong>{churches?.count ?? 0}</strong></article><article><small>수집 영상</small><strong>{sermons?.count ?? 0}</strong></article><article><small>검토할 익명 글</small><strong>{posts?.count ?? 0}</strong></article><article><small>검토할 달란트</small><strong>{talents?.count ?? 0}</strong></article></section>
