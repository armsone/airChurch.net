...
import { ChurchInfoEditControls, ReviewControls, ReviewerAccountControls } from "./admin-controls";
...
type EncouragementAdminRow={id:number;target_type:string;nickname:string;content:string;status:string;created_at:string;church_id:number;church_name:string;pastor:string};
...
export default async function AdminPage() {
  ...
  const [today, week, month, active, hourly, daily, monthly, churches, heldChurches, recommendations, pendingCommunity, publicChurchRows, heldChurchRows, postRows, talentRows, recommendationRows, contactRows] = await Promise.all([
    ...
    db.prepare("SELECT id,name,pastor,region,denomination,review_status,hold_reason,hold_note,held_at,priority_weight,homepage_url,youtube_channel_id,channel_image_url FROM churches WHERE review_status='approved' ORDER BY RANDOM() LIMIT 20").all<ChurchRow>(),
    db.prepare("SELECT id,name,pastor,region,denomination,review_status,hold_reason,hold_note,held_at,priority_weight,homepage_url,youtube_channel_id,channel_image_url FROM churches WHERE review_status='removed' ORDER BY RANDOM() LIMIT 10").all<ChurchRow>(),
  ...
  const [reviewerRows,changeRequestRows,pendingConcernRows,encouragementRows]=await Promise.all([
    ...
    db.prepare("SELECT e.id,e.target_type,e.nickname,e.content,e.status,e.created_at,e.church_id,c.name AS church_name,c.pastor FROM encouragement_messages e JOIN churches c ON c.id=e.church_id ORDER BY e.created_at DESC,e.id DESC").all<EncouragementAdminRow>(),
  ]);
  ...
  return <main className="admin-shell">
    ...
    <header className="admin-header">
      <HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true" /><span>airchurch</span></HomeReloadLink><div><span>관리자</span><a href="/pastor">목사님 페이지</a><form action="/api/admin/lock" method="post"><button type="submit">로그아웃</button></form></div>
    </header>
    ...
    <section className="admin-operations action-center-cards"><a className="admin-operation-card opinion" href="#reviewer-queue"><small>목사님 요청</small><strong>{changeRequestRows.results.length+pendingConcernGroups.length}</strong><span>{pendingRequestCount||pendingConcernGroups.length?`처리할 요청 ${pendingRequestCount+pendingConcernGroups.length}건 ↓`:"새 요청 없음"}</span></a><a className="admin-operation-card" href="#encouragement-messages"><small>응원글 전체</small><strong>{encouragementRows.results.length}</strong><span>교회·목사 응원글 관리 ↓</span></a><a className="admin-operation-card" href="#contact-requests"><small>운영 문의</small><strong>{pendingContactCount}</strong><span>권리자·정보 수정 요청 확인 ↓</span></a><a className="admin-operation-card" href="#pending-posts"><small>익명 글 검토</small><strong>{pendingCommunity?.count??0}</strong><span>새 글·신고 누적 글 결정 ↓</span></a><a className="admin-operation-card" href="#reviewer-accounts"><small>가입 승인 대기</small><strong>{pendingReviewerCount}</strong><span>목회자 계정 확인 ↓</span></a><a className="admin-operation-card" href="#church-recommendations"><small>교회 추천 검토</small><strong>{recommendations?.count ?? 0}</strong><span>등록 결정 ↓</span></a><a className="admin-operation-card" href="#church-management"><small>전체 교회</small><strong>{churches?.count ?? 0}</strong><span>검색·수정·보류·삭제 ↓</span></a></section>
    ...
    <section className="admin-section-heading" id="site-analytics"><span>SITE STATUS</span><h2>사이트 현황</h2><p>방문 흐름을 확인합니다.</p></section>
    ...
    <section className="admin-review-grid">
      <article className="admin-panel" id="encouragement-messages"><div className="admin-panel-title"><div><small>ENCOURAGEMENT</small><h2>교회·목사 응원글 전체</h2><p>공개 글과 보류 글을 함께 확인하고 보류·복원·삭제합니다.</p></div><span>{encouragementRows.results.length}건</span></div><div className="review-list">{encouragementRows.results.length?encouragementRows.results.map((item)=><article key={item.id}><div><span>{item.target_type==="pastor"?`${item.pastor} 목사`:`${item.church_name} 교회`} · {statusLabel(item.status)}</span><time>{koreanTime(item.created_at)}</time></div><strong>{item.nickname}</strong><p>{item.content}</p><a className="admin-review-link" href={item.target_type==="pastor"?`/pastors/${item.church_id}`:`/church/${item.church_id}`}>해당 페이지 보기 ↗</a><ReviewControls kind="encouragement" id={item.id} status={item.status}/></article>):<p className="admin-empty">아직 등록된 응원글이 없습니다.</p>}</div></article>
      ...
      <article className="admin-panel" id="pending-posts"><div className="admin-panel-title"><div><small>COMMUNITY REVIEW</small><h2>익명 글 관리</h2></div><span>{postRows.results.length}건</span></div><div className="review-list">{postRows.results.length ? postRows.results.map((post) => <article key={post.id}><div><span>{post.category} · {statusLabel(post.status)}{post.report_count?` · 신고 ${post.report_count}건`:""}</span><time>{koreanTime(post.created_at)}</time></div><strong>{post.nickname}</strong><p>{post.content}</p><ReviewControls kind="post" id={post.id} status={post.status} /></article>) : <p className="admin-empty">접수된 익명 글이 없습니다.</p>}</div></article>
