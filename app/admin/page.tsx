...
export default async function AdminPage() {
  ...
  const [reviewerRows,changeRequestRows]=await Promise.all([
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-operations action-center-cards"><a className="admin-operation-card opinion" href="#reviewer-queue"><small>목사님 요청</small><strong>{changeRequestRows.results.length}</strong><span>{pendingRequestCount?`새 요청 ${pendingRequestCount}건 · 먼저 처리 ↓`:"새 요청 없음"}</span></a><a className="admin-operation-card" href="#reviewer-accounts"><small>가입 승인 대기</small><strong>{pendingReviewerCount}</strong><span>목회자 계정 확인 ↓</span></a><a className="admin-operation-card" href="#church-recommendations"><small>교회 추천 검토</small><strong>{recommendations?.count ?? 0}</strong><span>등록 결정 ↓</span></a><a className="admin-operation-card" href="#church-management"><small>전체 교회</small><strong>{churches?.count ?? 0}</strong><span>검색·수정·보류·삭제 ↓</span></a></section>
