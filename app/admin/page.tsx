...
import { ChurchControls, ReviewControls, ReviewerAccountControls, ReviewerOpinionControls, SermonControls } from "./admin-controls";
...
export default async function AdminPage() {
  ...
  const [today, week, month, active, hourly, daily, monthly, paths, sermons, churches, posts, talents, recommendations, churchRows, sermonRows, postRows, talentRows, recommendationRows] = await Promise.all([
  ...
  const reviewerRows=await db.prepare("SELECT id,name,contact,username,status,created_at FROM reviewer_accounts ORDER BY CASE status WHEN 'pending' THEN 0 WHEN 'approved' THEN 1 ELSE 2 END,created_at DESC LIMIT 100").all<ReviewerAccountRow>();
  const reviewerOpinions=await db.prepare("SELECT r.id,r.church_id,COALESCE(a.name,CASE WHEN r.reviewer_id=0 THEN '기본 검토 계정' ELSE '삭제된 계정' END) AS reviewer_name,r.status,r.note,r.reviewed_at,r.handled_at FROM reviewer_church_reviews r LEFT JOIN reviewer_accounts a ON a.id=r.reviewer_id ORDER BY r.reviewed_at DESC LIMIT 1000").all<ReviewerOpinionRow>();
  ...
  const churchById=new Map(churchRows.results.map((church)=>[church.id,church]));
  const pendingOpinions=reviewerOpinions.results.filter((opinion)=>opinion.status!=="unreviewed"&&!opinion.handled_at&&churchById.has(opinion.church_id)).sort((a,b)=>a.status===b.status?b.reviewed_at.localeCompare(a.reviewed_at):a.status==="concern"?-1:1);
  const pendingConcernCount=pendingOpinions.filter((opinion)=>opinion.status==="concern").length;
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-operations"><a className="admin-operation-card opinion" href="#reviewer-queue"><small>목사님 의견 처리</small><strong>{pendingOpinions.length}</strong><span>{pendingConcernCount?`재검토 ${pendingConcernCount}건 · 지금 처리 ↓`:"지금 처리 ↓"}</span></a><a className="admin-operation-card" href="#church-management"><small>공개 교회</small><strong>{churches?.count ?? 0}</strong><span>수정·관리 ↓</span></a><a className="admin-operation-card" href="#church-recommendations"><small>검토할 교회 추천</small><strong>{recommendations?.count ?? 0}</strong><span>등록 결정 ↓</span></a><a className="admin-operation-card" href="#sermon-management"><small>공개 영상</small><strong>{sermons?.count ?? 0}</strong><span>긴급 관리 ↓</span></a><a className="admin-operation-card" href="#pending-posts"><small>검토할 익명 글</small><strong>{posts?.count ?? 0}</strong><span>검토하기 ↓</span></a><a className="admin-operation-card" href="#pending-talents"><small>검토할 달란트</small><strong>{talents?.count ?? 0}</strong><span>검토하기 ↓</span></a></section>

    <section className="admin-panel reviewer-queue" id="reviewer-queue"><div className="admin-panel-title"><div><small>PASTOR OPINIONS</small><h2>목사님 의견 처리</h2><p>새로 들어온 의견만 모았습니다. 내용을 확인하고 필요한 조치를 한 뒤 처리 완료를 눌러 주세요.</p></div><span>{pendingOpinions.length}건</span></div><div className="reviewer-queue-list">{pendingOpinions.length?pendingOpinions.map((opinion)=>{const church=churchById.get(opinion.church_id)!;return <article className={opinion.status==="concern"?"is-concern":""} key={opinion.id}><div className="admin-record-heading"><div><strong>{church.name}</strong><small>{church.pastor} · {church.region} · {church.denomination}</small></div><div className="admin-record-status"><span className={`reviewer-${opinion.status}`}>{reviewerLabel(opinion.status)}</span><span className={`status-${church.review_status}`}>{statusLabel(church.review_status)}</span></div></div><div className="reviewer-opinion-copy"><strong>{opinion.reviewer_name}</strong><time>{koreanTime(opinion.reviewed_at)}</time><p>{opinion.note||"별도 메모 없이 확인했습니다."}</p></div><ReviewerOpinionControls id={opinion.id}/><ChurchControls id={church.id} name={church.name} pastor={church.pastor} region={church.region} denomination={church.denomination} status={church.review_status} holdReason={church.hold_reason} holdNote={church.hold_note} heldAt={church.held_at} priorityWeight={church.priority_weight}/></article>}):<p className="admin-empty">처리할 목사님 의견이 없습니다.</p>}</div></section>

    <section className="admin-management-grid">
      <article className="admin-panel" id="church-management"><div className="admin-panel-title"><div><small>CHURCH MANAGEMENT</small><h2>공개 교회 관리</h2><p>목사님의 확인·재검토 의견을 먼저 표시합니다. 의견을 확인한 뒤 공개 유지 또는 보류를 관리자가 최종 결정해 주세요. <a href="/review">목사님 페이지 ↗</a></p></div><span>{publicChurchRows.length}곳</span></div><AdminListSearch targetId="public-church-list" total={publicChurchRows.length} label="공개 교회 검색" placeholder="교회명, 목사님, 지역, 교단 검색" /><div className="admin-manage-list" id="public-church-list">{publicChurchRows.length ? publicChurchRows.map((church) => <article key={church.id} data-admin-search={`${church.name} ${church.pastor} ${church.region} ${church.denomination} ${reviewerLabel(church.reviewer_status)} ${church.reviewer_note??""}`}><div className="admin-record-heading"><strong>{church.name}</strong><div className="admin-record-status"><span className={`reviewer-${church.reviewer_status}`}>{reviewerLabel(church.reviewer_status)}</span><span className={`priority-${church.priority_weight}`}>{church.priority_weight === 3 ? "♥ 매우 높음" : church.priority_weight === 2 ? "♥ 높음" : "기본"}</span><span className={`status-${church.review_status}`}>{statusLabel(church.review_status)}</span></div></div>{(opinionsByChurch.get(church.id)??[]).map((opinion,index)=><p className="admin-reviewer-note" key={`${opinion.reviewer_name}-${opinion.reviewed_at}-${index}`}>{opinion.reviewer_name} · {reviewerLabel(opinion.status)}{opinion.note?` · ${opinion.note}`:""}</p>)}<ChurchControls id={church.id} name={church.name} pastor={church.pastor} region={church.region} denomination={church.denomination} status={church.review_status} holdReason={church.hold_reason} holdNote={church.hold_note} heldAt={church.held_at} priorityWeight={church.priority_weight} /></article>) : <p className="admin-empty">공개 중인 교회가 없습니다.</p>}</div></article>
    ...
    <section className="admin-review-grid">
      <article className="admin-panel" id="reviewer-accounts"><div className="admin-panel-title"><div><small>REVIEWER ACCOUNTS</small><h2>목회자 검토자 가입</h2><p>성함과 연락처를 확인한 뒤 교회 목록 검토 권한을 승인해 주세요.</p></div><span>{reviewerRows.results.length}명</span></div><div className="review-list">{reviewerRows.results.length?reviewerRows.results.map((item)=><article key={item.id}><div><span>{statusLabel(item.status)}</span><time>{koreanTime(item.created_at)}</time></div><strong>{item.name} · {item.username}</strong><p>{item.contact}</p><ReviewerAccountControls id={item.id} status={item.status}/></article>):<p className="admin-empty">목회자 가입 신청이 없습니다.</p>}</div></article>
