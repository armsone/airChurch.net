...
export default async function PastorPhotoReviewPage(){
  ...
  return <main className="admin-shell admin-subpage">
    <header className="admin-header"><a className="brand" href="/admin"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></a><div><span>관리자 · 목회자 사진</span></div></header>
    <section className="admin-title"><div><span>PASTOR PHOTOS</span><h1>공식 목회자 사진 확인</h1><p>교회·교단이 공식 공개한 목회자 단독 프로필은 공개할 수 있습니다. 가족·신도 등 제3자나 사적 사진은 제외하고, 문제 제기가 접수되면 먼저 비노출합니다.</p></div><a href="/admin">관리자로 돌아가기</a></section>
    <section className="admin-panel"><div className="admin-panel-title"><div><small>REVIEW QUEUE</small><h2>사진 검토 대기</h2></div><span>대기 {Number(count?.count??0).toLocaleString("ko-KR")}건 · 100건 표시</span></div><div className="review-list admin-photo-review-grid">{rows.results.length?rows.results.map((item)=>{const photo=safeHttpUrl(item.photo_url),source=safeHttpUrl(item.photo_source_url);return <article key={item.id}>{photo&&<img src={photo} alt={`${item.name} 목회자 사진 검토`} width={96} height={116} loading="lazy" decoding="async" referrerPolicy="no-referrer"/>}<div><span>{item.photo_sha256?`파일 해시 ${item.photo_sha256.slice(0,12)}…`:"파일 해시 확인 중"}</span><time>{time(item.updated_at)}</time></div><strong>{item.name}</strong>{source&&<a className="admin-review-link" href={source} target="_blank" rel="noreferrer">공식 이름표·출처 확인 ↗</a>}<PastorPhotoControls id={item.id}/></article>}):<p className="admin-empty">검토 대기 중인 공식 사진이 없습니다.</p>}</div></section>
  </main>;
}
