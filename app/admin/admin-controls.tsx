...
export function ReviewControls({ kind, id, status }: { kind: "post" | "talent" | "recommendation" | "contact" | "encouragement"; id: number; status: string }) {
  const [busy, setBusy] = useState(false), [error, setError] = useState("");
  async function setStatus(next: "pending" | "approved" | "rejected" | "deleted") {
    if (next === "rejected" && !window.confirm("공개하지 않고 반려할까요?")) return;
    if (next === "deleted") {
      const label=kind === "recommendation" ? "교회 추천" : kind === "post" ? "익명 글" : kind === "contact" ? "문의" : kind==="encouragement"?"응원글":"달란트";
  ...
  const approveLabel=kind === "recommendation" ? "교회 등록 승인" : kind === "contact" ? "처리 완료" : kind==="encouragement"?"공개로 복원":"공개 승인";
  const rejectLabel=kind === "recommendation" ? "등록하지 않음" : kind === "contact" ? "처리하지 않음" : "비공개";
  return <div className="admin-action-row"><button type="button" disabled={busy || status === "approved"} className="restore" onClick={() => void setStatus("approved")}>{approveLabel}</button>{kind!=="encouragement"&&<button type="button" disabled={busy || status === "rejected"} className="danger" onClick={() => void setStatus("rejected")}>{rejectLabel}</button>}{status !== "pending" && <button type="button" disabled={busy} onClick={() => void setStatus("pending")}>{kind==="encouragement"?"보류":"재검토"}</button>}<button type="button" disabled={busy} className="danger" onClick={() => void setStatus("deleted")}>삭제</button>{error && <span className="admin-error">{error}</span>}</div>;
}
