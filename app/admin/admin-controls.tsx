...
export function ChurchControls(props: { id: number; name: string; pastor: string; region: string; denomination: string; status: string; holdReason: string | null; holdNote: string | null; heldAt: string | null; priorityWeight: number }) {
  ...
  return <details className="admin-church-details"><summary><span className="admin-church-details-open" aria-label="교회 정보 및 공개 상태 관리">정보 관리</span><span className="admin-church-details-close" aria-label="관리 화면 닫기">관리 닫기</span><b aria-hidden="true">⌄</b></summary><form className="admin-edit-form" onSubmit={save}>
