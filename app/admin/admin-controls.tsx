...
export function PastorControls(props:PastorControlProps){
  ...
  return <details ref={detailsRef} className="admin-pastor-details"><summary className="sr-only">목회자 정보와 공개 상태 관리</summary><div className="admin-pastor-editor-heading"><strong>{props.name} 목회자 관리</strong><button type="button" onClick={()=>{if(detailsRef.current)detailsRef.current.open=false;}} aria-label={`${props.name} 목회자 관리 닫기`}>닫기 ×</button></div><form className="admin-edit-form" onSubmit={save}><div className="admin-edit-fields"><input name="name" defaultValue={props.name} aria-label="목회자 이름" required/><input name="roleTitle" defaultValue={props.roleTitle} aria-label="직분" required/><input name="churchName" defaultValue={props.churchName??""} aria-label="소속 교회" required/><input name="region" defaultValue={props.region??""} aria-label="지역" required/><input name="denomination" defaultValue={props.denomination??""} aria-label="목회자 교단" required/></div><div className="admin-preference-fields"><label><span>사역 상태</span><select name="roleStatus" defaultValue={props.roleStatus==="former"?"former":"current"}><option value="current">현재 사역</option><option value="former">이전 사역</option></select></label></div><div className="admin-action-row"><button type="submit" disabled={busy||!props.roleId}>정보 저장</button><a href="/admin/pastor-photos">사진 검토 열기 →</a></div>{!props.roleId&&<p className="admin-error">연결된 사역 관계가 없어 이 카드에서는 정보를 수정할 수 없습니다.</p>}{error&&<p className="admin-error" role="alert">{error}</p>}</form><div className="admin-pastor-status-actions"><span>공개 상태</span><ReviewControls kind="pastor-person" id={props.id} status={props.status}/></div></details>;
}
