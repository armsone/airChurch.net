...
export default function ChurchRequestResolution({id}:{id:number}) {
  ...
  async function decide(resolution:"approved"|"rejected"|"deferred") {
    if((resolution==="rejected"||resolution==="deferred")&&note.trim().length<3){setError("목사님이 볼 답변을 3자 이상 적어 주세요.");return;}
  ...
  return <div className="church-request-resolution"><label className="admin-note-field"><span>목사님에게 남길 답변 <small>· 반려·보류 시 필수</small></span><textarea value={note} onChange={(event)=>setNote(event.target.value)} maxLength={500} rows={3} placeholder="결정 이유나 다시 확인할 내용을 적어 주세요."/></label><div className="admin-action-row"><button className="restore" disabled={busy} onClick={()=>void decide("approved")}>그대로 승인</button><button className="danger" disabled={busy} onClick={()=>void decide("rejected")}>반려</button><button disabled={busy} onClick={()=>void decide("deferred")}>일단 보류</button></div>{error&&<p className="admin-error">{error}</p>}</div>;
}
