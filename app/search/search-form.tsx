...
export default function SearchForm({q,region,denomination,regions,denominations}:{q:string;region:string;denomination:string;regions:string[];denominations:string[]}){
  ...
  return <>
    <form action="/search" method="get" onSubmit={remember}>
      <label htmlFor="result-search-query"><span aria-hidden="true">⌕</span><span className="sr-only">교회, 목회자, 지역, 교단 또는 말씀·찬양 검색</span><input id="result-search-query" name="q" list="result-search-suggestions" type="search" inputMode="search" enterKeyHint="search" aria-describedby="result-search-status" autoComplete="off" autoCapitalize="none" spellCheck={false} value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="교회, 목회자, 지역, 교단 또는 말씀·찬양 제목"/></label>
      ...
      <span className="sr-only" id="result-search-status" role="status" aria-live="polite">{suggestions.length?`자동완성 ${suggestions.length}개가 있습니다.`:"교회, 목회자, 지역, 교단을 함께 검색할 수 있습니다."}</span>
      <select name="region" defaultValue={region||"전체"} aria-label="지역">{regions.map((item)=><option key={item}>{item}</option>)}</select>
      <select name="denomination" defaultValue={denomination||"전체 교단"} aria-label="교단">{denominations.map((item)=><option key={item}>{item}</option>)}</select>
