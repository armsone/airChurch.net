...
export default function SearchForm({q,region,denomination,regions,denominations}:{q:string;region:string;denomination:string;regions:string[];denominations:string[]}){
  ...
  const remember=()=>{const term=query.trim(),normalized=normalizeSearchValue(term);if(!normalized)return;const next=[term,...recent.filter((item)=>normalizeSearchValue(item)!==normalized)].slice(0,5);setRecent(next);try{localStorage.setItem("airchurch:recent-searches",JSON.stringify(next));}catch{/* 저장이 제한된 브라우저에서도 검색은 계속합니다. */}};
  return <><form action="/search" method="get" onSubmit={remember}><label htmlFor="result-search-query"><span aria-hidden="true">⌕</span><span className="sr-only">교회, 목사, 지역, 교단 또는 말씀·찬양 검색</span><input id="result-search-query" name="q" list="result-search-suggestions" autoComplete="off" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="교회, 목사, 지역, 교단 또는 말씀·찬양 제목"/></label><datalist id="result-search-suggestions">{suggestions.map((item)=><option value={item.value} key={`${item.value}-${item.label}`}>{item.label}</option>)}</datalist><select name="region" defaultValue={region||"전체"} aria-label="지역">{regions.map((item)=><option key={item}>{item}</option>)}</select><select name="denomination" defaultValue={denomination||"전체 교단"} aria-label="교단">{denominations.map((item)=><option key={item}>{item}</option>)}</select><button type="submit">통합 검색</button></form>{recent.length>0&&<div className="search-recent"><span>최근 검색</span>{recent.map((item)=><a href={`/search?q=${encodeURIComponent(item)}`} key={item}>{item}</a>)}<button type="button" onClick={()=>{setRecent([]);localStorage.removeItem("airchurch:recent-searches");}}>지우기</button></div>}</>;
}
