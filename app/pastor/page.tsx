...
export default async function PastorPage() {
  ...
  const [churchRows,featuredRows,requestRows]=await Promise.all([
    db.prepare("SELECT id,name,pastor,region,denomination,review_status,homepage_url,youtube_channel_id,channel_image_url FROM churches WHERE review_status IN ('approved','removed') ORDER BY name LIMIT 2000").all<Church>(),
    db.prepare("SELECT id,name,pastor,region,denomination,review_status,homepage_url,youtube_channel_id,channel_image_url FROM churches WHERE review_status IN ('approved','removed') ORDER BY RANDOM() LIMIT 20").all<Church>(),
