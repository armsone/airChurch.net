...
export default function ChurchRequestManager({churches,featuredChurches,requests}:{churches:Church[];featuredChurches:Church[];requests:RequestItem[]}) {
  const [query,setQuery]=useState(""),[selectedIds,setSelectedIds]=useState<Set<number>>(new Set()),[action,setAction]=useState<keyof typeof actionLabels>("edit"),[busy,setBusy]=useState(false),[message,setMessage]=useState("");
  const results=useMemo(()=>{const needle=query.trim().toLocaleLowerCase("ko-KR");if(!needle)return featuredChurches;return churches.filter((church)=>`${church.name} ${church.pastor} ${church.region} ${church.denomination}`.toLocaleLowerCase("ko-KR").includes(needle));},[churches,featuredChurches,query]);
