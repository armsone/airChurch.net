...
export async function ensureSearchDiscoveryPools(db:ReturnType<typeof import("./api/_shared").database>){
  await db.prepare("CREATE TABLE IF NOT EXISTS search_discovery_pool_v2 (kind TEXT NOT NULL,bucket INTEGER NOT NULL,position INTEGER NOT NULL,item_key TEXT NOT NULL,PRIMARY KEY(kind,bucket,position))").run();
  ...
  for(const [table,kinds,updateWhen] of triggerTables)for(const action of ["INSERT","UPDATE","DELETE"] as const){
    const trigger=`search_discovery_v2_${table}_${action.toLowerCase()}`,quoted=kinds.map((kind)=>`'${kind}'`).join(",");
    await db.prepare(`CREATE TRIGGER IF NOT EXISTS ${trigger} AFTER ${action} ON ${table}${action==="UPDATE"?` WHEN ${updateWhen}`:""} BEGIN DELETE FROM search_discovery_pool_v2 WHERE kind IN (${quoted}); END`).run();
  }
  for(const kind of Object.keys(sources) as DiscoveryKind[]){
    const ready=await db.prepare("SELECT COUNT(DISTINCT bucket) AS buckets,COUNT(*) AS items FROM search_discovery_pool_v2 WHERE kind=?").bind(kind).first<{buckets:number;items:number}>();
    if(Number(ready?.buckets)===50&&Number(ready?.items)>0)continue;
    const source=sources[kind];
    await db.prepare("DELETE FROM search_discovery_pool_v2 WHERE kind=?").bind(kind).run();
    await db.prepare(`WITH RECURSIVE buckets(bucket) AS (SELECT 0 UNION ALL SELECT bucket+1 FROM buckets WHERE bucket<49), ranked AS (SELECT buckets.bucket,CAST(source.${source.id} AS TEXT) AS item_key,ROW_NUMBER() OVER (PARTITION BY buckets.bucket ORDER BY (((source.${source.id}*(1103515245+((buckets.bucket+1)*104729)))+((buckets.bucket+1)*12345))&2147483647),source.${source.id}) AS position FROM ${source.table} source CROSS JOIN buckets WHERE ${source.where}) INSERT INTO search_discovery_pool_v2(kind,bucket,position,item_key) SELECT ?,bucket,position,item_key FROM ranked WHERE position<=40`).bind(kind).run();
  }
}

export function discoveryFilter(kind:DiscoveryKind,bucket:number,column:string){
  return ` AND CAST(${column} AS TEXT) IN (SELECT item_key FROM search_discovery_pool_v2 WHERE kind='${kind}' AND bucket=${bucket})`;
}

export function discoveryOrder(kind:DiscoveryKind,bucket:number,column:string){
  return `(SELECT position FROM search_discovery_pool_v2 WHERE kind='${kind}' AND bucket=${bucket} AND item_key=CAST(${column} AS TEXT))`;
}
