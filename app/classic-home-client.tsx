...
export default function Home() {
  ...
  const [praiseTab,setPraiseTab]=useState<"ccm"|"church"|"bible-music"|"holy-verse">("ccm");
  ...
  const [sermonOffset,setSermonOffset]=useState(0);
  const [rankingRefresh,setRankingRefresh]=useState(0),[rankingBusy,setRankingBusy]=useState(false);
  const [postRefresh,setPostRefresh]=useState(0),[postBusy,setPostBusy]=useState(false);
  const [expandedRankings,setExpandedRankings]=useState({churches:false,pastors:false});
  ...
  useEffect(()=>{
    ...
    const refreshSaved=()=>queueMicrotask(()=>setSavedItems(readSavedItems()));
    window.addEventListener("focus",refreshPersonalState);window.addEventListener("storage",refreshPersonalState);window.addEventListener("airchurch:saved-change",refreshSaved);
    return()=>{window.removeEventListener("focus",refreshPersonalState);window.removeEventListener("storage",refreshPersonalState);window.removeEventListener("airchurch:saved-change",refreshSaved);};
  },[todayKey]);
  useEffect(()=>{
  ...
  useEffect(()=>{const controller=new AbortController();setRankingBusy(true);fetch(`/api/rankings?refresh=${rankingRefresh}`,{signal:controller.signal,cache:"no-store"}).then(response=>{if(!response.ok)throw Error();return response.json();}).then(result=>{if(!controller.signal.aborted)setRankings({churches:Array.isArray(result.churches)?result.churches:[],pastors:Array.isArray(result.pastors)?result.pastors:[]});}).catch(()=>{if(!controller.signal.aborted)setNotice("순위를 새로 확인하지 못했습니다. 이전 순위를 유지합니다.");}).finally(()=>{if(!controller.signal.aborted)setRankingBusy(false);});return()=>controller.abort();},[rankingRefresh]);
  useEffect(()=>{if(!postRefresh)return;const controller=new AbortController();setPostBusy(true);Promise.all(["posts","talents"].map(kind=>fetch(`/api/${kind}?refresh=${postRefresh}`,{signal:AbortSignal.any([controller.signal,AbortSignal.timeout(8000)]),cache:"no-store"}).then(response=>{if(!response.ok)throw Error();return response.json();}))).then(([posts,talents])=>{if(!controller.signal.aborted){setApprovedPosts(posts.items||[]);setApprovedTalents(talents.items||[]);}}).catch(()=>{if(!controller.signal.aborted)setNotice("광장 소식을 새로 확인하지 못했습니다. 잠시 후 다시 눌러 주세요.");}).finally(()=>{if(!controller.signal.aborted)setPostBusy(false);});return()=>controller.abort();},[postRefresh]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    const loaders: Record<string, () => Promise<unknown>> = {
      ...
      "pastor-directory": ()=>{const fresh=sessionStorage.getItem("airchurch:pastor-cache-bust"),params=new URLSearchParams({limit:String(lowData?8:12),sample:String(pastorBucketRef.current),photoMix:"1"});if(fresh)params.set("adminFresh",fresh);return loadItems(`/api/pastors?${params}`).then((data)=>{
        if(!alive) return;
    ...
    ["sermons","church-news","church-directory","community"].forEach(loadSection);
  ...
  const sermonStart=filtered.length?sermonOffset%filtered.length:0;
  const visibleSermons = [...filtered.slice(sermonStart),...filtered.slice(0,sermonStart)].slice(0,12);
  ...
  async function showDifferentPastors(){
    setPastorLoading(true);setPastorVisibleCount(12);
    pastorBucketRef.current=(pastorBucketRef.current+1)%50;const params=new URLSearchParams({limit:"12",sample:String(pastorBucketRef.current),photoMix:"1"}),fresh=sessionStorage.getItem("airchurch:pastor-cache-bust");if(pastorTotal>0)params.set("knownTotal",String(pastorTotal));if(fresh)params.set("adminFresh",fresh);
  ...
  function toggleSaved(item:SavedItem) {
    const current=readSavedItems(),exists=current.some(saved=>saved.id===item.id);
  ...
  return (
    <main id="top" className="integrated-portal">
      ...
      <section className="hero" id="primary-content" tabIndex={-1}>
        ...
        <form className="search" role="search" action="/search" method="get" onSubmit={()=>{const term=query.trim(),normalized=normalizeSearchValue(term);if(normalized){const next=[term,...recentSearches.filter((item)=>normalizeSearchValue(item)!==normalized)].slice(0,5);setRecentSearches(next);try{writeRecentSearches(next);}catch{/* 저장이 제한된 브라우저에서도 검색은 계속합니다. */}}}}>
          ...
          <div className="search-filters">
            <select name="region" aria-label="지역 선택" value={region} onChange={(e) => { setRegion(e.target.value);setShowAllChurches(false); }}>{regions.map((item) => <option key={item}>{item}</option>)}</select>
            <select name="denomination" className="denomination-filter" aria-label="교단 선택" value={denomination} onChange={(e) => { setDenomination(e.target.value);setShowAllChurches(false); }}>{denominationOptions.map((item) => <option key={item}>{item}</option>)}</select>
      ...
      <section className="content-section" id="sermons">
        ...
        <div className="sermon-grid sermon-preview-grid">
          {sermonLoading ? <LoadingCards count={12} /> : visibleSermons.map((sermon, index) => <article className="sermon-card" id={index === visibleSermons.length - 1 ? "sermons-end" : undefined} key={sermon.id}>
      ...
      <section className="content-section praise-section" id="praises">
        ...
        <div className="holyverse-introduction"><a className="holyverse-sample" href="https://airchurch.net/?ccm=aE_pL0M14d0#praises" aria-label="홀리벌스 찬양 샘플 듣기"><img src="https://i.ytimg.com/vi/aE_pL0M14d0/hqdefault.jpg" width={480} height={360} alt="" loading="lazy"/><span aria-hidden="true">▶</span></a><div><span className="section-kicker">HOLY VERSE</span><h3>홀리벌스 · 말씀을 랩과 노래로</h3><p>샘플 한 곡부터 듣고, 더 많은 찬양을 만나보세요.</p><div className="holyverse-actions"><a href="https://airchurch.net/?ccm=aE_pL0M14d0#praises">샘플 듣기 →</a><button type="button" onClick={()=>{setPraiseTab("holy-verse");setActiveVideoId(null);}}>홀리벌스 전체 듣기</button><a href="https://www.youtube.com/playlist?list=PLqiTHwLF3Dbb4LSoWz6NfzG8UbprRH4fE" target="_blank" rel="noopener noreferrer">공식 재생목록 ↗</a></div></div></div>
      ...
      <section className="content-section shorts-section" id="shorts">
        ...
        <div className="shorts-grid shorts-home-grid">
          {shortLoading ? <LoadingCards count={12} /> : visibleShorts.map((short) => <button className="shorts-card" type="button" key={short.youtubeId} onClick={(event)=>openRandomShort(short,event.currentTarget)} aria-label={`${short.church} 쇼츠 ${short.title} 재생`}>
      ...
      <section className="interest-ranking content-section" id="interest-ranking" aria-labelledby="interest-ranking-title">
        <div className="section-heading"><div><span className="section-kicker">ANONYMOUS INTEREST</span><h3 id="interest-ranking-title">이번 주 많이 찾은 교회와 목회자</h3><p>최근 7일의 익명 방문 기록을 기준으로 소개합니다.</p></div><button className="unified-other-button" type="button" disabled={rankingBusy} onClick={()=>setRankingRefresh(value=>value+1)}>{rankingBusy?"확인 중…":"최신 순위 확인"}</button></div>
      ...
      <section className="church-directory-section" id="church-directory">
        ...
        <div className="church-directory-search" role="search">
          ...
          <input
            ...
            placeholder="등록된 교회명, 목회자, 지역, 교단으로 찾아보세요"
      ...
      <section className="community-section community-reading" id="community">
        ...
        {approvedPosts.length > 0 || approvedTalents.length > 0 ? <div className="approved-list">{[...approvedPosts.map(post=>({kind:"post" as const,id:post.id,title:post.nickname,content:post.content,createdAt:post.createdAt,post})),...approvedTalents.map(talent=>({kind:"talent" as const,id:talent.id,title:talent.title,content:talent.description,createdAt:talent.createdAt,post:null}))].sort((a,b)=>b.createdAt.localeCompare(a.createdAt)).map(item=><article key={`${item.kind}:${item.id}`} id={item.kind==="post"?`community-post-${item.id}`:`community-talent-${item.id}`}><h3>{item.title}</h3><p>{item.content}</p>{item.post&&<button className="community-report" type="button" aria-label={`${item.title} 글을 운영자에게 신고`} onClick={()=>void reportPost(item.post!)}>원칙에 맞지 않는 글 신고</button>}</article>)}</div> : <p className="community-empty">아직 공개된 글이 없습니다. 이야기, 기도, 나누고 싶은 달란트를 남겨주세요.</p>}
