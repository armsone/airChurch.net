...
const ensurePastorPublicIdsV28=async(db:D1Database)=>{
  const columns=await db.prepare("PRAGMA table_info(pastor_people)").all<{name:string}>();
  await addColumnIfMissing(db,columns.results,"public_id","ALTER TABLE pastor_people ADD COLUMN public_id INTEGER");
  await db.batch([
    db.prepare("UPDATE pastor_people SET public_id=0 WHERE id=(SELECT p.id FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE REPLACE(TRIM(p.name),' ','')='정성진' AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='거룩한빛광성교회' ORDER BY CASE WHEN p.review_status='approved' THEN 0 ELSE 1 END,p.id LIMIT 1)"),
    db.prepare("UPDATE pastor_people SET public_id=1 WHERE id=(SELECT p.id FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE REPLACE(TRIM(p.name),' ','')='곽승현' AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='거룩한빛광성교회' ORDER BY CASE WHEN p.review_status='approved' THEN 0 ELSE 1 END,p.id LIMIT 1)"),
    db.prepare("UPDATE pastor_people SET public_id=2 WHERE id=(SELECT p.id FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE REPLACE(TRIM(p.name),' ','')='김민석' AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='거룩한빛광성교회' ORDER BY CASE WHEN p.review_status='approved' THEN 0 ELSE 1 END,p.id LIMIT 1)"),
    db.prepare("UPDATE pastor_people SET public_id=3 WHERE id=(SELECT p.id FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE REPLACE(TRIM(p.name),' ','')='이일현' AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='거룩한빛광성교회' ORDER BY CASE WHEN p.review_status='approved' THEN 0 ELSE 1 END,p.id LIMIT 1)"),
    db.prepare(`WITH candidates AS (SELECT p.id,p.name,MIN(CASE r.role_category WHEN 'current_primary' THEN 0 WHEN 'associate' THEN 1 WHEN 'education' THEN 2 WHEN 'cooperating' THEN 3 WHEN 'emeritus' THEN 4 ELSE 5 END) AS role_rank FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE p.public_id IS NULL AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='거룩한빛광성교회' GROUP BY p.id,p.name),ranked AS (SELECT id,ROW_NUMBER() OVER(ORDER BY role_rank,name,id) AS rn FROM candidates) UPDATE pastor_people SET public_id=1000+(SELECT rn FROM ranked WHERE ranked.id=pastor_people.id) WHERE id IN (SELECT id FROM ranked)`),
    db.prepare("CREATE UNIQUE INDEX IF NOT EXISTS idx_pastor_people_public_id ON pastor_people(public_id)"),
    db.prepare("DROP TRIGGER IF EXISTS trg_pastor_people_public_id"),
    db.prepare("CREATE TRIGGER IF NOT EXISTS trg_kwangsung_pastor_public_id AFTER INSERT ON pastor_church_roles WHEN REPLACE(TRIM(COALESCE(NEW.church_name,'')),' ','')='거룩한빛광성교회' AND (SELECT public_id FROM pastor_people WHERE id=NEW.pastor_id) IS NULL BEGIN UPDATE pastor_people SET public_id=(SELECT MAX(COALESCE(MAX(public_id),0),1000)+1 FROM pastor_people WHERE public_id<1000000) WHERE id=NEW.pastor_id; END"),
...
const ensureChurchPrimaryPastorsV29=async(db:D1Database)=>{
  ...
  await db.batch([
    db.prepare("DROP TRIGGER IF EXISTS trg_pastor_people_public_id"),
    db.prepare("CREATE TRIGGER trg_pastor_people_public_id AFTER INSERT ON pastor_people WHEN NEW.public_id IS NULL BEGIN UPDATE pastor_people SET public_id=(SELECT MAX(COALESCE(MAX(public_id),0),1000)+1 FROM pastor_people WHERE id<>NEW.id AND public_id IS NOT NULL) WHERE id=NEW.id; END"),
    db.prepare(`WITH unassigned AS (SELECT id,ROW_NUMBER() OVER(ORDER BY id) AS rn FROM pastor_people WHERE public_id IS NULL),base AS (SELECT MAX(COALESCE(MAX(public_id),0),1000) AS max_id FROM pastor_people WHERE public_id IS NOT NULL) UPDATE pastor_people SET public_id=(SELECT base.max_id+unassigned.rn FROM unassigned,base WHERE unassigned.id=pastor_people.id) WHERE id IN (SELECT id FROM unassigned)`),
...
export const ensurePastorPeopleTables=memoizeEnsure(async(db:D1Database)=>{
  ...
  await addColumnIfMissing(db,personColumns.results,"public_id","ALTER TABLE pastor_people ADD COLUMN public_id INTEGER");
  ...
  await db.prepare(`${kwangsungStaff} INSERT INTO pastor_people(directory_id,name,review_status) SELECT 'kwangsung-official-'||name,name,'approved' FROM staff WHERE CASE WHEN staff.name='김민석' THEN NOT EXISTS (SELECT 1 FROM pastor_people p WHERE p.directory_id='kwangsung-official-김민석') ELSE NOT EXISTS (SELECT 1 FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE p.review_status='approved' AND r.review_status='approved' AND REPLACE(TRIM(p.name),' ','')=REPLACE(staff.name,' ','') AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='거룩한빛광성교회') END`).run();
  await db.prepare(`${kwangsungStaff} INSERT OR IGNORE INTO pastor_church_roles(pastor_id,church_id,church_name,denomination,region,role_title,role_category,role_status,source_url,review_status) SELECT p.id,c.id,c.name,c.denomination,c.region,staff.role_title,staff.role_category,'current','https://kwangsung.org/Page/Index/15','approved' FROM staff JOIN pastor_people p ON REPLACE(TRIM(p.name),' ','')=REPLACE(staff.name,' ','') AND (staff.name<>'김민석' OR p.directory_id='kwangsung-official-김민석') JOIN churches c ON REPLACE(TRIM(c.name),' ','')='거룩한빛광성교회' WHERE p.review_status='approved' AND NOT EXISTS (SELECT 1 FROM pastor_church_roles r WHERE r.pastor_id=p.id AND r.review_status='approved' AND r.church_id=c.id)`).run();
  ...
  await db.batch(jejakwangsungOfficialPastors.map((pastor)=>db.prepare(`INSERT OR IGNORE INTO pastor_people(directory_id,name,review_status) SELECT 'jejakwangsung-official-'||REPLACE(?,' ',''),?,'approved' WHERE NOT EXISTS (SELECT 1 FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE p.review_status='approved' AND r.review_status='approved' AND REPLACE(TRIM(p.name),' ','')=REPLACE(?,' ','') AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='제자광성교회')`).bind(pastor.name,pastor.name,pastor.name)));
  await db.batch(jejakwangsungOfficialPastors.map((pastor)=>db.prepare(`INSERT OR IGNORE INTO pastor_church_roles(pastor_id,church_id,church_name,denomination,region,role_title,role_category,role_status,source_url,review_status) SELECT p.id,c.id,c.name,c.denomination,c.region,?,?,'current',?,'approved' FROM pastor_people p JOIN churches c ON REPLACE(TRIM(c.name),' ','')='제자광성교회' WHERE p.review_status='approved' AND REPLACE(TRIM(p.name),' ','')=REPLACE(?,' ','') AND NOT EXISTS (SELECT 1 FROM pastor_church_roles r WHERE r.pastor_id=p.id AND r.review_status='approved' AND r.church_id=c.id) ORDER BY CASE WHEN p.directory_id LIKE 'jejakwangsung-official-%' THEN 0 ELSE 1 END,p.id LIMIT 1`).bind(pastor.roleTitle,pastor.roleCategory,pastor.sourceUrl,pastor.name)));
  await db.batch(jejakwangsungOfficialPastors.map((pastor)=>db.prepare(`UPDATE pastor_people SET photo_url=?,photo_source_url=?,photo_usage_basis='official_public_clergy_profile',photo_review_status='approved',updated_at=CURRENT_TIMESTAMP WHERE id IN (SELECT DISTINCT p.id FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE p.review_status='approved' AND r.review_status='approved' AND REPLACE(TRIM(p.name),' ','')=REPLACE(?,' ','') AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='제자광성교회' )`).bind(pastor.photoUrl,pastor.sourceUrl,pastor.name)));
