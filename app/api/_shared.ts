...
const ensureChurchPrimaryPastorsV29=async(db:D1Database)=>{
  ...
  await db.batch([
    db.prepare("DROP TRIGGER IF EXISTS trg_pastor_people_public_id"),
    db.prepare("CREATE TRIGGER trg_pastor_people_public_id AFTER INSERT ON pastor_people WHEN NEW.public_id IS NULL BEGIN UPDATE pastor_people SET public_id=(SELECT MAX(COALESCE(MAX(public_id),0),10000)+1 FROM pastor_people WHERE id<>NEW.id AND public_id IS NOT NULL) WHERE id=NEW.id; END"),
