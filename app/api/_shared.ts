...
export const ensurePastorPeopleTables=memoizeEnsure(async(db:D1Database)=>{
  await ensureMaintenanceState(db);
  const ready=await db.prepare("SELECT key FROM maintenance_state WHERE key='schema-pastor-people-v3' LIMIT 1").first<{key:string}>();if(ready)return;
  await db.batch([
    db.prepare("CREATE TABLE IF NOT EXISTS pastor_people (id INTEGER PRIMARY KEY AUTOINCREMENT,directory_id TEXT UNIQUE,name TEXT NOT NULL,public_summary TEXT,photo_url TEXT,photo_source_url TEXT,photo_sha256 TEXT,photo_review_status TEXT NOT NULL DEFAULT 'pending',review_status TEXT NOT NULL DEFAULT 'pending',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
  ...
  await db.prepare("INSERT OR REPLACE INTO maintenance_state (key,completed_at) VALUES ('schema-pastor-people-v3',CURRENT_TIMESTAMP)").run();
});
...
export const ensureAdminTables=memoizeEnsure(async(db:D1Database)=>{
  const keys=["schema-analytics-v1","schema-community-v1","schema-contact-v1","schema-sermons-v5","schema-praises-v1","schema-shorts-v1","schema-recommendations-v1","schema-reviewers-v3","schema-private-contacts-v1","schema-encouragement-v2","schema-ministry-profiles-v3","schema-pastor-people-v3"];
