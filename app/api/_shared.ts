...
export const ensurePastorPeopleTables=memoizeEnsure(async(db:D1Database)=>{
  await ensureMaintenanceState(db);
  const ready=await db.prepare("SELECT key FROM maintenance_state WHERE key='schema-pastor-people-v24' LIMIT 1").first<{key:string}>();if(ready)return;
  await db.batch([
  ...
  await db.batch(jejakwangsungOfficialPastors.map((pastor)=>db.prepare(`UPDATE pastor_church_roles SET role_title=?,role_category=?,role_status='current',source_url=?,updated_at=CURRENT_TIMESTAMP WHERE id=(SELECT r.id FROM pastor_church_roles r JOIN pastor_people p ON p.id=r.pastor_id WHERE r.review_status='approved' AND p.review_status='approved' AND REPLACE(TRIM(p.name),' ','')=REPLACE(?,' ','') AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='제자광성교회' ORDER BY CASE WHEN p.directory_id LIKE 'jejakwangsung-official-%' THEN 0 ELSE 1 END,p.id,r.id LIMIT 1)`).bind(pastor.roleTitle,pastor.roleCategory,pastor.sourceUrl,pastor.name)));
  await db.batch(jejakwangsungOfficialPastors.map((pastor)=>db.prepare(`UPDATE pastor_people SET photo_url=?,photo_source_url=?,photo_usage_basis='official_public_clergy_profile',photo_review_status='approved',updated_at=CURRENT_TIMESTAMP WHERE id=(SELECT p.id FROM pastor_people p JOIN pastor_church_roles r ON r.pastor_id=p.id WHERE p.review_status='approved' AND r.review_status='approved' AND REPLACE(TRIM(p.name),' ','')=REPLACE(?,' ','') AND REPLACE(TRIM(COALESCE(r.church_name,'')),' ','')='제자광성교회' ORDER BY CASE WHEN p.directory_id LIKE 'jejakwangsung-official-%' THEN 0 ELSE 1 END,p.id LIMIT 1)`).bind(pastor.photoUrl,pastor.sourceUrl,pastor.name)));
