...
export async function GET() {
  ...
  const items=selectWeightedRecent(result.results.filter((item)=>isSermonTitle(item.title)),30).map((item)=>({youtubeId:item.youtubeId,title:item.title,thumbnailUrl:item.thumbnailUrl,publishedAt:item.publishedAt,church:item.church,pastor:item.pastor,region:item.region,denomination:item.denomination}));
