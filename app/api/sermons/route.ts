...
export async function GET() {
  ...
  const items=selectWeightedRecent(result.results.filter((item)=>isSermonTitle(item.title)),120).map((item)=>({youtubeId:item.youtubeId,title:item.title,thumbnailUrl:`https://i.ytimg.com/vi/${item.youtubeId}/mqdefault.jpg`,publishedAt:item.publishedAt,church:item.church,pastor:item.pastor,region:item.region,denomination:item.denomination}));
  scheduleSync();
