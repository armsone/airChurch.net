...
export async function GET(request:Request) {
  ...
  const rawCursor=(params.get("pageToken")||"").slice(0,200),[section,rawOffset,...tokenParts]=rawCursor.split("|");
  const isPsalms=section==="psalms",offset=Number.isSafeInteger(Number(rawOffset))?Number(rawOffset):0,pageToken=tokenParts.join("|");
  ...
  const limit=isPsalms?159:200;
  const items=(playlist.items||[]).flatMap((item)=>{const id=item.contentDetails?.videoId,title=item.snippet?.title?.trim();return id&&title?[{id,title,channel:channelName,duration:0}]:[];}).slice(0,Math.max(0,limit-offset));
  const nextOffset=offset+items.length;
  const nextCursor=nextOffset<limit&&playlist.nextPageToken?`${isPsalms?"psalms":"shorts"}|${nextOffset}|${playlist.nextPageToken}`:isPsalms?null:"psalms|0|";
