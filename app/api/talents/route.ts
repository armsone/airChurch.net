...
export async function POST(request: Request) {
  ...
  const db=database(); await ensureCommunityTables(db); const fp=await fingerprint(request,"talent-offer"); const recent=await db.prepare("SELECT COUNT(*) AS count FROM talent_offers WHERE fingerprint = ? AND created_at > datetime('now', '-10 minutes')").bind(fp).first<{count:number}>(); if((recent?.count||0)>=2) return Response.json({error:"잠시 후 다시 시도해 주세요."},{status:429,headers:{"cache-control":"no-store","retry-after":"600"}});
  await db.prepare("INSERT INTO talent_offers (title, region, description, fingerprint) VALUES (?, ?, ?, ?)").bind(title,region,description,fp).run(); return Response.json({ok:true,status:"pending"},{status:201,headers:{"cache-control":"no-store"}});
}
