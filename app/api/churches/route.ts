...
export async function GET() {
  ...
  const result=await db.prepare("SELECT id,name,pastor,region,denomination FROM churches WHERE review_status='approved' ORDER BY priority_weight DESC,name LIMIT 300").all();
