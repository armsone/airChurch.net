...
const normalize=(value:string)=>value.toLowerCase().replace(/[^\p{L}\p{N}]/gu,"").replace(/목사(?:님)?$/,""),expand=(term:string)=>denominationAliases[normalize(term)]??[normalize(term)];
...
export async function GET(request:Request) {
  ...
  const haystack="replace(replace(replace(replace(replace(replace(lower(name||pastor||region||denomination),' ',''),'-',''),'·',''),'.',''),'(',''),')','')";
