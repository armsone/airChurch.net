...
export async function POST(request: Request) {
  ...
  const db=database(); await ensureCommunityTables(db); const fp=await fingerprint(request,"community-post"); const recent=await db.prepare("SELECT COUNT(*) AS count FROM community_posts WHERE fingerprint = ? AND created_at > datetime('now', '-10 minutes')").bind(fp).first<{count:number}>(); if((recent?.count||0)>=2) return Response.json({error:"잠시 후 다시 시도해 주세요."},{status:429,headers:{"cache-control":"no-store","retry-after":"600"}});
