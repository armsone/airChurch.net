...
export async function GET(request:NextRequest){
  ...
  `).bind(normalized,term,term,term,term,term,region,regionTerm,denomination,denomination,normalized,term,term,term,term,term,region,regionTerm,denomination,denomination,normalized,term,term,term,term,term,region,regionTerm,denomination,denomination,limit,offset).all<PastorRow>();
  return NextResponse.json({items:rows.results,total:Number(rows.results[0]?.total_count??0),offset,limit},{headers:{"cache-control":"public, max-age=60, s-maxage=300, stale-while-revalidate=900"}});
}
