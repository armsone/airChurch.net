...
type PastorRow={person_id:number|null;role_id:number|null;church_id:number|null;minister_id:number|null;name:string;role_title:string;role_titles:string;role_status:string;church_name:string|null;region:string|null;denomination:string|null;photo_url:string|null;merged_count:number;total_count:number};
...
export async function GET(request:NextRequest){
  ...
  const rows=await db.prepare(`
    WITH raw_people AS (
      SELECT p.id person_id,r.id role_id,r.church_id,NULL minister_id,p.name,r.role_title,r.role_status,r.church_name,r.region,r.denomination,CASE WHEN p.photo_review_status='approved' THEN p.photo_url ELSE NULL END photo_url
      ...
      UNION ALL
      SELECT NULL,NULL,c.id,NULL,c.pastor,'담임목사','current',c.name,c.region,c.denomination,NULL FROM churches c
      ...
      UNION ALL
      SELECT NULL,NULL,c.id,m.id,m.name,m.role_title,m.role_status,c.name,c.region,c.denomination,NULL FROM church_ministry_profiles m JOIN churches c ON c.id=m.church_id
