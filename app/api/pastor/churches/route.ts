...
export async function GET(request:Request){
  ...
  const [rows,count]=await Promise.all([
    db.prepare(`SELECT id,name,pastor,region,denomination,review_status,homepage_url,youtube_channel_id,channel_image_url FROM churches WHERE ${where} ORDER BY ${order} LIMIT ? OFFSET ?`).bind(...bindings,...(groups.length?relevance.bindings:[]),pageSize,offset).all<ChurchRow>(),
