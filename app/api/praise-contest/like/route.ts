...
export async function POST(request:Request) {
  ...
  try {
    ...
    const db=database();
    const previous=await db.prepare("SELECT entry_id AS entryId FROM praise_contest_votes WHERE contest_id=? AND browser_hash=?").bind(CONTEST.id,identity.hash).first<{entryId:number}>();
    if(previous)return json({ok:previous.entryId===id,votedEntryId:previous.entryId,error:previous.entryId===id?undefined:"이 브라우저에서는 이미 좋아요에 참여했습니다."},previous.entryId===id?200:409);
    const inserted=await db.prepare(`INSERT OR IGNORE INTO praise_contest_votes(contest_id,entry_id,browser_hash,created_at)
      SELECT ?,e.id,?,strftime('%Y-%m-%dT%H:%M:%fZ','now') FROM praise_contest_entries e
    ...
    if(!inserted.meta.changes){const vote=await db.prepare("SELECT entry_id AS entryId FROM praise_contest_votes WHERE contest_id=? AND browser_hash=?").bind(CONTEST.id,identity.hash).first<{entryId:number}>();return json({error:vote?"이 브라우저에서는 이미 좋아요에 참여했습니다.":"마감되었거나 현재 참여할 수 없는 영상입니다.",votedEntryId:vote?.entryId??null},409);}
