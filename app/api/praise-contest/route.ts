...
import { browser, entriesWithRanks, json, maintainContest, resolvedContestPhase, validMutation } from "./_server";
export const dynamic = "force-dynamic";
export async function GET(request: Request) {
  try {
    await maintainContest();
    const phase=await resolvedContestPhase(), identity=await browser(request,phase==="open"||phase==="voting");
    ...
    return json({items:phase==="cancelled"?items.map(item=>({...item,rank:null,prize:0})):items,phase,serverNow:new Date().toISOString(),votedEntryId:vote?.entryId??null,finalized:phase==="finished"},200,identity?.cookie);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const inserted=await db.prepare(`INSERT INTO praise_contest_entries(contest_id,youtube_id,performer,title,channel_name,contact,source_file_url,browser_hash,consent_version,consent_at,status,reupload_status,created_at)
      ...
      WHERE strftime('%Y-%m-%dT%H:%M:%fZ','now')>=? AND strftime('%Y-%m-%dT%H:%M:%fZ','now')<?`).bind(CONTEST.id,videoId,performer,title,clean(metadata.author_name,120),contact,sourceFileUrl,identity.hash,CONTEST.consentVersion,"not_requested",CONTEST.startsAt,CONTEST.submissionEndsAt).run();
