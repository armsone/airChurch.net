import { clean, consumeSubmissionLimit, database, ensureContactTables, fingerprint, readLimitedJson, requestOriginIsInvalid } from "../_shared";
...
export async function POST(request:Request) {
  ...
  if(!await consumeSubmissionLimit(db,"contact",fp,3,24*60)) return Response.json({error:"오늘 보낼 수 있는 문의 수를 초과했습니다. 내일 다시 시도해 주세요."},{status:429,headers:{"cache-control":"no-store","retry-after":"86400"}});
  await db.prepare("INSERT INTO contact_requests (category,name,contact,message,fingerprint) VALUES (?,?,?,?,?)").bind(category,name,contact,message,"").run();
