import { clean,consumeSubmissionLimit,database,ensureEncouragementTables,ensureMinistryProfileTables,ensureSermonTables,fingerprint,readLimitedJson,requestOriginIsInvalid } from "../_shared";
...
export async function POST(request:Request){
  ...
  if(targetType==="pastor"&&targetRef.startsWith("minister:")&&!await db.prepare("SELECT id FROM church_ministry_profiles WHERE id=? AND church_id=? AND review_status='approved' LIMIT 1").bind(Number(targetRef.slice(9)),churchId).first())return Response.json({error:"현재 응원할 수 없는 목회자입니다."},{status:404});
  const inserted=await db.prepare("INSERT INTO encouragement_messages (church_id,target_type,target_ref,nickname,content,status) VALUES (?,?,?,?,?,'approved')").bind(churchId,targetType,targetRef,nickname,content).run();
