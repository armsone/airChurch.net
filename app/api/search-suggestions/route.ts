...
const normalize=(value:string)=>value.toLowerCase().replace(/[^\p{L}\p{N}]/gu,"").replace(/목사(?:님)?$/,""),expand=(term:string)=>aliases[normalize(term)]??[normalize(term)];
...
export async function GET(request:Request){
  ...
  const groups=terms.map(expand),haystack="replace(replace(replace(replace(replace(replace(lower(name||pastor||region||denomination),' ',''),'-',''),'·',''),'.',''),'(',''),')','')";
  ...
  const result=await db.prepare(`SELECT name,pastor,region,denomination,priority_weight AS priorityWeight FROM churches WHERE review_status='approved' AND ${conditions} LIMIT 60`).bind(...groups.flat()).all<SuggestionRow>();
  ...
  return Response.json({items},{headers:{"cache-control":"public, max-age=180, s-maxage=180, stale-while-revalidate=900"}});
}
