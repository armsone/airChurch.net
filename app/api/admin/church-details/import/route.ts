...
import { clean,database,ensureChurchDetailTables,ensureSermonTables,readLimitedJson,requestOriginIsInvalid } from "../../../_shared";
...
export async function POST(request:Request){
  ...
  const parsed:Array<{action:"schedule"|"profile";churchId:number;values:Record<string,string|null>}>=[];
  ...
  const db=database();await Promise.all([ensureSermonTables(db),ensureChurchDetailTables(db)]);
