...
export async function PATCH(request:Request){
 ...
 if(!Number.isSafeInteger(id)||id<1||!["published","held"].includes(status)||!["awaiting_source","rights_review","ready","uploaded"].includes(reuploadStatus))return json({error:"입력 값을 확인해 주세요."},400);
 ...
 try{
  const db=database();const before=await db.prepare("SELECT status FROM praise_contest_entries WHERE id=? AND contest_id=?").bind(id,CONTEST.id).first<{status:string}>();if(!before)return json({error:"참가 영상을 찾을 수 없습니다."},404);
