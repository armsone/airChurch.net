...
type PastorRow={id:number;name:string;review_status:string;photo_url:string|null;photo_review_status:string;role_title:string|null;role_status:string|null;church_name:string|null;region:string|null;denomination:string|null;source_url:string|null};
...
export async function GET(request:Request){
  ...
  const conditions=groups.map((group)=>`(${group.map(()=>`instr(${haystack},?)>0`).join(" OR ")})`),bindings=groups.flat(),where=conditions.length?`WHERE ${conditions.join(" AND ")}`:"";
  ...
  const [rows,count]=await Promise.all([
    db.prepare(`SELECT p.id,p.name,p.review_status,p.photo_url,p.photo_review_status,r.role_title,r.role_status,r.church_name,r.region,r.denomination,r.source_url FROM pastor_people p ${roleJoin} ${where} ORDER BY ${order} LIMIT ? OFFSET ?`).bind(...bindings,...(groups.length?relevance.bindings:[]),pageSize,offset).all<PastorRow>(),
