...
import { clean,database,ensurePrivateContactTables,ensureSermonTables,readLimitedJson,requestOriginIsInvalid } from "../../_shared";
...
export async function POST(request:Request){
  ...
  const church=await db.prepare("SELECT id FROM churches WHERE id=? LIMIT 1").bind(churchId).first();if(!church)return Response.json({error:"교회를 찾을 수 없습니다."},{status:404});
  const [encrypted,digest]=await Promise.all([encryptPrivateContact(value),digestPrivateContact(value)]);
  await db.prepare("INSERT INTO private_church_contacts (church_id,contact_type,encrypted_value,value_digest,scope,source_url,review_status) VALUES (?,?,?,?,?,?,'approved') ON CONFLICT(church_id,contact_type,value_digest) DO UPDATE SET encrypted_value=excluded.encrypted_value,scope=excluded.scope,source_url=excluded.source_url,review_status='approved',updated_at=CURRENT_TIMESTAMP").bind(churchId,type,encrypted,digest,scope,source.toString()).run();
