import {clean,consumeSubmissionLimit,database,ensureMinistryProfileTables,ensureSermonTables,fingerprint,readLimitedJson,requestOriginIsInvalid} from "../_shared";
...
export async function POST(request:Request){
  ...
  if(!await db.prepare("SELECT id FROM churches WHERE id=? AND review_status='approved' LIMIT 1").bind(churchId).first())return Response.json({error:"현재 제보할 수 없는 교회입니다."},{status:404});
