...
export async function POST(request:Request) {
  ...
  const name=clean(data.name,80),contact=clean(data.contact,120),username=clean(data.username,40).toLowerCase(),churchName=clean(data.churchName,100);
  const password=typeof data.password==="string"?data.password:"";
  if(name.length<2||contact.length<5||churchName.length<2||!/^[a-z0-9._-]{4,40}$/.test(username)||password.length<10||password.length>128) return Response.json({error:"성함·연락처·섬기는 교회, 4자 이상의 영문 아이디와 10자 이상의 비밀번호를 확인해 주세요."},{status:400});
  ...
  const churches=await db.prepare(`SELECT id FROM churches WHERE review_status='approved' AND ${sqlNormalized("name")}=? ORDER BY id LIMIT 2`).bind(normalizeSearchValue(churchName)).all<{id:number}>();
  if(churches.results.length!==1)return Response.json({error:"등록 교회에서 정확히 한 곳을 확인할 수 없습니다. 교회명을 다시 확인해 주세요."},{status:400});
  const {hash,salt}=await hashReviewerPassword(password);
  await db.prepare("INSERT INTO reviewer_accounts (name,contact,username,password_hash,password_salt,status,fingerprint,church_id) VALUES (?,?,?,?,?,'pending',?,?)").bind(name,contact,username,hash,salt,"",churches.results[0].id).run();
