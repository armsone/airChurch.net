...
export type MakingPost = typeof posts[number] & { status: string; updatedAt?: string; sortAt?: string };
...
function fromRow(row: Row): MakingPost {
  return { slug: row.slug, title: row.title, summary: row.summary, category: row.category, sections: JSON.parse(row.body), sources: JSON.parse(row.sources), status: row.status, date: row.created_at.slice(0,10), updatedAt: row.updated_at.slice(0,10), sortAt: row.created_at.replace(" ","T") };
}
export async function listMakingPosts(includeHidden = false) {
  ...
  return [...all.values()].filter(post => includeHidden || post.status === "published").sort((a,b) => (b.sortAt??b.date).localeCompare(a.sortAt??a.date));
}
