...
export default function MakingHomeSection(){
  ...
  return <section ref={ref} className="making-home" aria-labelledby="making-home-title"><div className="making-home-intro"><h2 id="making-home-title">에어처치 만들기</h2><p>사이트를 만들며 조사하고 고민한 내용을 한 편씩 남깁니다.</p><a href="/making">전체 글 보기 →</a></div><div className="making-home-posts">{items.length?items.map(item=><a key={item.slug} href={`/making/${item.slug}`}><span>{item.category} · <time dateTime={item.date}>{item.date.replaceAll("-", ".")}</time></span><h3>{item.title}</h3></a>):<p role="status">{message}</p>}<nav aria-label="제작 기록 더 보기"><a href="/tech">테크</a><a href="/history">히스토리</a></nav></div></section>;
}
