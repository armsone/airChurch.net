...
function reviewLabel(status:string){return status==="confirmed"?"특이사항 없음":status==="concern"?"검토 의견 보냄":"아직 확인 전";}
...
function ChurchCard({church}:{church:ChurchReviewRow}) {
  return <article data-admin-search={`${church.name} ${church.pastor} ${church.region} ${church.denomination} ${reviewLabel(church.reviewer_status)}`}><div className="reviewer-church-heading"><div><span>{church.region}</span><h3>{church.name}</h3><p><strong>{pastorLabel(church.pastor)}</strong><span>{church.denomination}</span></p></div><div><span className={`reviewer-status status-${church.reviewer_status}`}>{reviewLabel(church.reviewer_status)}</span>{church.review_status==="removed"&&<span className="reviewer-status status-removed">사이트 보류 중</span>}{church.youtube_channel_id&&<a href={`https://www.youtube.com/channel/${church.youtube_channel_id}`} target="_blank" rel="noreferrer">공식 채널 ↗</a>}</div></div>{church.reviewer_status==="concern"&&<p className={`review-process-status ${church.handled_at?"is-handled":""}`}>{church.handled_at?"운영팀이 의견을 확인했습니다":"운영팀이 의견을 살펴보고 있습니다"}</p>}<ChurchReviewControls id={church.id} status={church.reviewer_status} note={church.reviewer_note} reviewedAt={church.reviewed_at}/></article>;
}
...
export default async function ReviewPage() {
  ...
  const reviewerName=role==="admin"?"관리자":reviewerAccount?.name??"목사님";
  ...
  return <main className="admin-shell reviewer-shell"><header className="admin-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink><div><span>{role==="admin"?"관리자 검토 모드":`${reviewerName}님`}</span>{role==="admin"&&<a href="/admin">전체 관리</a>}<form action="/api/admin/lock" method="post"><button type="submit">로그아웃</button></form></div></header>
    <section className="admin-title"><div><span>교회 검토</span><h1>{reviewerName}님, 한 곳씩 살펴봐 주세요</h1><p>특이사항이 없으면 바로 기록하고, 염려되는 점이 있으면 알고 계신 내용과 함께 의견을 보내 주세요.</p></div><HomeReloadLink>에어처치 보기 ↗</HomeReloadLink></section>
    <section className="reviewer-metrics"><a href="#review-todo"><small>아직 살펴볼 교회</small><strong>{todoTotal}</strong><span>검토 시작 ↓</span></a><a className="concern" href="#review-concern"><small>내가 보낸 검토 의견</small><strong>{concernTotal}</strong><span>진행 상황 보기 ↓</span></a><a href="#review-done"><small>특이사항 없음</small><strong>{doneTotal}</strong><span>확인 내역 ↓</span></a></section>
    <div id="review-todo"><QuickReviewQueue key={`${todo[0]?.id??0}-${todo.length}-${todoTotal}`} todo={todo} total={todoTotal}/></div>
    <section className="admin-panel reviewer-panel reviewer-history"><div className="admin-panel-title"><div><small>내 검토 내역</small><h2>내가 살펴본 교회</h2><p>보낸 의견의 진행 상황과 특이사항 없이 확인한 교회를 다시 볼 수 있습니다.</p></div><span>{concernTotal+doneTotal}곳</span></div><AdminListSearch targetId="reviewer-history-lists" total={concerns.length+done.length} label="살펴본 교회 검색" placeholder="교회명, 목사님, 지역, 교단 검색"/>
      <div id="reviewer-history-lists">
        <section className="review-section" id="review-concern"><div className="review-section-title"><div><small>운영팀에 전한 내용</small><h2>검토 의견을 보낸 교회</h2></div><strong>{concernTotal}곳</strong></div><div className="reviewer-church-list">{concerns.length?concerns.map((church)=><ChurchCard church={church} key={church.id}/>):<p className="admin-empty">아직 보낸 검토 의견이 없습니다.</p>}</div></section>
        <details className="review-section review-done" id="review-done"><summary><span><small>확인을 마친 교회</small><b>특이사항이 없었던 교회</b></span><strong>{doneTotal}곳 · 펼쳐 보기</strong></summary><div className="reviewer-church-list">{done.length?done.map((church)=><ChurchCard church={church} key={church.id}/>):<p className="admin-empty">아직 특이사항 없이 확인한 교회가 없습니다.</p>}</div></details>
      </div>
    </section>
  </main>;
}
