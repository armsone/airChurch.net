...
function ChurchCard({church}:{church:ChurchReviewRow}) {
  return <article data-admin-search={`${church.name} ${church.pastor} ${church.region} ${church.denomination} ${reviewLabel(church.reviewer_status)}`}><div className="reviewer-church-heading"><div><span>{church.region}</span><h3>{church.name}</h3><p><strong>{pastorLabel(church.pastor)}</strong><span>{church.denomination}</span></p></div><div><span className={`reviewer-status status-${church.reviewer_status}`}>{reviewLabel(church.reviewer_status)}</span>{church.review_status==="removed"&&<span className="reviewer-status status-removed">보류 중</span>}{church.youtube_channel_id&&<a href={`https://www.youtube.com/channel/${church.youtube_channel_id}`} target="_blank" rel="noreferrer">공식 채널 ↗</a>}</div></div>{church.reviewer_status==="concern"&&<p className={`review-process-status ${church.handled_at?"is-handled":""}`}>{church.handled_at?"관리자가 확인했어요":"관리자 확인을 기다리고 있어요"}</p>}<ChurchReviewControls id={church.id} status={church.reviewer_status} note={church.reviewer_note} reviewedAt={church.reviewed_at}/></article>;
}
