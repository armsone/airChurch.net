...
export default function CtsDiscovery({compact=false}:{compact?:boolean}){
  ...
  const visible=[...filtered.slice(start),...filtered.slice(0,start)].slice(0,compact?2:expanded?40:4);
  useEffect(()=>{if(compact)return;const follow=()=>{if(window.location.hash==="#faith-stories")setExpanded(true);};follow();window.addEventListener("hashchange",follow);return()=>window.removeEventListener("hashchange",follow);},[compact]);
  ...
  return <section className={compact?"portal-panel faith-preview":"cts-discovery"} id={compact?undefined:"faith-stories"} aria-labelledby={compact?"faith-preview-title":"cts-discovery-title"}>
    <div className="portal-panel-heading"><div>{!compact&&<span className="section-kicker">에어처치가 모은 신앙 이야기</span>}<h3 id={compact?"faith-preview-title":"cts-discovery-title"}>신앙이야기</h3></div>{compact?<a href="#faith-stories">더 보기 →</a>:<button className="faith-refresh" type="button" disabled={refreshing} onClick={()=>void refresh(true)}>{refreshing?"확인 중…":"↻ 새로고침"}</button>}</div>
    <p className="portal-caption">삶의 고백, 청년의 질문, 성경 속 이야기. 마음에 닿는 영상부터 만나보세요.</p>
    {!compact&&<div className="portal-switch cts-switch" aria-label="신앙 영상 주제">{categories.map(name=><button key={name} type="button" aria-pressed={category===name} onClick={()=>{setCategory(name);setPlaying(null);setExpanded(false);setVideoOffset(0);}}>{name}</button>)}</div>}
    ...
    {!compact&&filtered.length>4&&<button className="faith-more" type="button" onClick={()=>setVideoOffset(value=>value+4)}>다른 이야기 보기</button>}
    {!compact&&filtered.length>4&&<button className="faith-more" type="button" aria-expanded={expanded} onClick={()=>setExpanded(value=>!value)}>{expanded?"접기":"영상 더 보기"}</button>}
    {!compact&&<p className="portal-health" role="status">{refreshing?"새 영상을 확인하고 있습니다. ":checkedAt?`마지막 확인 ${new Date(checkedAt).toLocaleString("ko-KR",{timeZone:"Asia/Seoul",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"})} · `:""}{feedDelayed?"새 영상 확인이 지연되어 확보된 영상을 표시합니다.":"CBS·CGN·바이블프로젝트 공식 채널의 새 영상을 주기적으로 확인합니다."} CTS는 2026. 9. 11. 확인한 영상 모음입니다.</p>}
  </section>;
}
