...
export default function CtsDiscovery({compact=false,dedicated=false}:{compact?:boolean;dedicated?:boolean}){
  ...
  return <section className={compact?"portal-panel faith-preview":"cts-discovery"} id={compact?undefined:"faith-stories"} aria-labelledby={compact?"faith-preview-title":"cts-discovery-title"}>
    <div className="portal-panel-heading"><div><h3 id={compact?"faith-preview-title":"cts-discovery-title"}>신앙이야기</h3></div>{compact?<a href="#faith-stories">더 보기 →</a>:dedicated?<button className="faith-refresh" type="button" disabled={refreshing} onClick={()=>void refresh(true)}>{refreshing?"확인 중…":"새 영상 확인"}</button>:<a href="/faith-stories">전체 보기 →</a>}</div>
    ...
    {!compact&&!dedicated&&filtered.length>4&&<button className="faith-more" type="button" onClick={()=>setVideoOffset(value=>value+4)}>다른 이야기 보기</button>}
