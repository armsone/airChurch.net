...
export default function CtsDiscovery({compact=false,dedicated=false}:{compact?:boolean;dedicated?:boolean}){
  ...
  return <section className={compact?"portal-panel faith-preview portal-tone-story":dedicated?"cts-discovery":"cts-discovery faith-home"} id={compact?undefined:"faith-stories"} aria-labelledby={compact?"faith-preview-title":"cts-discovery-title"}>
    ...
    {dedicated&&<p className="portal-health" role="status">{refreshing?"새 영상을 확인하고 있습니다. ":checkedAt?`마지막 확인 ${new Date(checkedAt).toLocaleString("ko-KR",{timeZone:"Asia/Seoul",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"})} · `:""}{feedDelayed?"일부 채널의 새 영상 확인이 지연되어 확보된 영상을 함께 표시합니다.":"CTS·CBS·CGN·바이블프로젝트 공식 채널의 새 영상을 15분마다 확인합니다."} CTS는 ‘내가 매일 기쁘게’·‘두란노 성경교실’의 새 영상과 기존 다시보기 모음을 함께 제공합니다.</p>}
  </section>;
}
