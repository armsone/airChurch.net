...
export async function boundedFetch(url:string,source:SourceConfig,pace?:()=>Promise<void>,robots?:string):Promise<{text:string;status:number;finalUrl:string}> {
  for(let redirect=0;redirect<4;redirect++){
    ...
    while(true){const {value,done}=await reader.read();if(done)break;size+=value.byteLength;if(size>responseLimit){void reader.cancel();throw Error(`response_too_large:received=${size};declared=${declaredSize}`);}chunks.push(value);}
