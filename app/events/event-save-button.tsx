...
export default function EventSaveButton({ item }: { item:Pick<ChurchEvent,"id"|"title"|"startDate"|"endDate"|"startTime"|"venue"> }) {
  ...
  const toggle=()=>{
    ...
    writeSavedItems(exists?current.filter(value=>value.id!==id):[{id,kind:"event",title:item.title.slice(0,300),subtitle:subtitle.slice(0,500),url:`/events/${item.id}`,savedAt:new Date().toISOString()},...current]);
