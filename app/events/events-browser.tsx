...
export default function EventsBrowser({ compact=false, preview=false, churchId, portalRegion, onPortalRegionChange }: { compact?:boolean; preview?:boolean; churchId?:number; portalRegion?:string; onPortalRegionChange?:(value:string)=>void }) {
  ...
  if(preview)return <div ref={container}><p className="portal-caption">{portalRegion&&portalRegion!=="전체"?portalRegion:"전국"} · 가까운 일정</p>{loading?<p className="portal-empty">일정을 불러오는 중입니다.</p>:<><ul className="portal-picks">{shown.slice(0,3).map(item=><li key={item.id}><a href={`/events/${item.id}`}><strong>{item.title}</strong><small>{dateLabel(item.startDate)} · {item.venue||item.region}</small></a></li>)}</ul>{error&&<p className="portal-health">일정 확인이 지연되고 있습니다. <button type="button" onClick={()=>setRevision(value=>value+1)}>다시 시도</button></p>}{!error&&!shown.length&&<p className="portal-empty">확인된 예정 행사가 없습니다.</p>}</>}</div>;
  return <div className="events-browser" ref={container}>
    {compact&&<div className="section-heading"><div><span className="section-kicker">함께하는 신앙</span><h2>{title}</h2><p>공식 공지에서 확인한 행사 일정입니다. 정확한 진행일과 참여 방법은 원문에서 확인해 주세요.</p></div><button className="unified-other-button" type="button" disabled={loading} onClick={()=>setRevision(value=>value+1)}>{loading?"확인 중…":"일정 새로고침"}</button><a className="church-news-shuffle unified-other-button" href={churchId?`/events?church=${churchId}`:"/events"}>전체 일정 보기 →</a></div>}
