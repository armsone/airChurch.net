...
import ChurchVideoLink from "./church-video-link";
...
export default async function ChurchPage({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="church-detail-shell"><SkipLink/>
    ...
    <section className="church-detail-hero" id="primary-content" tabIndex={-1}><div className="church-detail-identity">{image?<img src={image} alt="" width={96} height={96} loading="eager" decoding="async" referrerPolicy="no-referrer"/>:<span aria-hidden="true">교회</span>}<div><small>확인된 공식 정보</small><h1>{church.name}</h1><p>{church.pastor} · {church.region} · {church.denomination}</p></div></div><div className="church-detail-actions"><ChurchSaveButton id={church.id} name={church.name} pastor={church.pastor} region={church.region}/><ChurchShareButton name={church.name}/>{homepage&&<a href={homepage} target="_blank" rel="noopener noreferrer">공식 홈페이지 ↗</a>}{church.youtube_channel_id&&<a href={`https://www.youtube.com/channel/${church.youtube_channel_id}`} target="_blank" rel="noopener noreferrer">공식 YouTube ↗</a>}</div></section>
