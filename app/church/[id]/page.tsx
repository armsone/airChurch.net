...
export default async function ChurchPage({params}:{params:Promise<{id:string}>}){
  ...
  const [sermons,praises,related,profile,schedules,encouragements,ministries,personMinistries]=await Promise.all([
    ...
    db.prepare("SELECT p.id AS person_id,p.name,r.role_title,r.role_category,r.role_status,r.source_url FROM pastor_church_roles r JOIN pastor_people p ON p.id=r.pastor_id WHERE r.church_id=? AND r.review_status='approved' AND p.review_status='approved' ORDER BY CASE r.role_category WHEN 'current_primary' THEN 0 WHEN 'associate' THEN 1 WHEN 'education' THEN 2 WHEN 'cooperating' THEN 3 WHEN 'emeritus' THEN 4 ELSE 5 END,p.name LIMIT 80").bind(id).all<PersonMinistryRow>(),
  ]);
  ...
  return <main className="church-detail-shell"><SkipLink/>
    <header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink><nav><a href="/#sermons">말씀</a><a href="/#church-directory">교회 찾기</a><a href="/pastors">목회자</a><SavedNavLink/><a href="/about">운영 안내</a></nav><a className="church-detail-back" href="/#church-directory">목록으로</a></header>
    <section className="church-detail-hero" id="primary-content" tabIndex={-1}><div className="church-detail-identity">{image?<img src={image} alt="" width={96} height={96} loading="eager" decoding="async" referrerPolicy="no-referrer"/>:<span aria-hidden="true">교회</span>}<div><small>확인된 공식 정보</small><h1>{church.name}</h1><p><a className="church-pastor-profile-link" href={primaryPerson?`/pastors/p/${primaryPerson.person_id}`:`/pastors/${church.id}`}>{church.pastor} 목회 기록 보기 →</a> · {church.region} · {church.denomination}</p></div></div><div className="church-detail-actions"><ChurchSaveButton id={church.id} name={church.name} pastor={church.pastor} region={church.region}/><ChurchShareButton name={church.name}/>{homepage&&<a href={homepage} target="_blank" rel="noopener noreferrer">공식 홈페이지 ↗</a>}{church.youtube_channel_id&&<a href={`https://www.youtube.com/channel/${church.youtube_channel_id}`} target="_blank" rel="noopener noreferrer">공식 YouTube ↗</a>}</div></section>
