...
type PersonMinistryRow={person_id:number;public_id:number;name:string;role_title:string;role_category:string;role_status:string;source_url:string|null};
...
export default async function ChurchPage({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="church-detail-shell"><SkipLink/>
    ...
    {ministryCards.length>0&&<section className="church-detail-content church-ministry"><div className="section-heading"><div><span className="section-kicker">공식 출처 확인</span><h2>함께 섬긴 목회자와 교역자</h2></div><span className="result-count">{ministryCards.length}명</span></div><div className="church-ministry-grid">{ministryCards.map((minister)=><article className="pastor-home-card shared-pastor-card" key={`${minister.href}-${minister.name}-${minister.role_title}`}><PastorCardContent name={minister.name.replace(/\s*목사(?:님)?$/u,"")} href={minister.href} photoUrl="/pastor-silhouette-soft.png" roleStatus={minister.role_status} roles={[minister.role_title]} churchName={church.name} churchHref={`/church/${church.id}`} region={church.region} denomination={church.denomination} sourceUrl={minister.source_url}/></article>)}</div></section>}
    ...
    {related.results.length>0&&<section className="church-detail-content church-related"><div className="section-heading"><div><span className="section-kicker">다음 발견</span><h2>가까운 교회와 같은 교단</h2></div><a href="/#church-directory">전체 교회 찾기 →</a></div><div className="church-related-grid">{related.results.map((item)=>{const reasons=[item.region.split(/\s+/)[0]===church.region.split(/\s+/)[0]?"같은 지역":null,item.denomination===church.denomination?"같은 교단":null].filter(Boolean);return <article className="shared-church-card" key={item.id}><ChurchCardContent id={item.id} name={item.name} pastor={item.pastor} region={item.region} denomination={item.denomination} detail={<small className="church-related-reason">{reasons.join(" · ")}</small>}/></article>})}</div></section>}
