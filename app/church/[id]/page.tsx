...
export default async function ChurchPage({params}:{params:Promise<{id:string}>}){
  ...
  const [sermons,praises,related,profile,schedules,encouragements,ministries,personMinistries]=await Promise.all([
    ...
    db.prepare("SELECT id,name,pastor,region,denomination,priority_weight FROM churches WHERE review_status='approved' AND id!=? AND (region LIKE ? OR denomination=?) ORDER BY RANDOM() LIMIT 6").bind(id,regionPrefix,church.denomination).all<RelatedChurch>(),
  ...
  return <main className="church-detail-shell"><SkipLink/>
    ...
    <section className="church-detail-hero" id="primary-content" tabIndex={-1}><div className="church-detail-identity">{image?<img src={image} alt="" width={96} height={96} loading="eager" decoding="async" referrerPolicy="no-referrer"/>:<span aria-hidden="true">교회</span>}<div><small>확인된 공식 정보</small><h1>{church.name}</h1><p><a className="church-pastor-profile-link" href={primaryPerson?`/pastors/${primaryPerson.public_id}`:`/church/${church.id}`}>{church.pastor} 목회 기록 보기 →</a> · {church.region} · {church.denomination}</p></div></div><div className="church-detail-actions"><ChurchSaveButton id={church.id} name={church.name} pastor={church.pastor} region={church.region}/><ChurchShareButton name={church.name}/>{homepage&&<a href={homepage} target="_blank" rel="noopener noreferrer">공식 홈페이지 ↗</a>}{church.youtube_channel_id&&<a href={`https://www.youtube.com/channel/${church.youtube_channel_id}`} target="_blank" rel="noopener noreferrer">공식 YouTube ↗</a>}<EncouragementJumpLink/></div></section>
    ...
    {ministryCards.length>0&&<section className="church-detail-content church-ministry"><div className="section-heading"><div><span className="section-kicker">공식 출처 확인</span><h2>함께 섬긴 목회자와 교역자</h2></div><span className="result-count">{ministryCards.length}명</span></div><div className="church-ministry-grid">{ministryCards.map((minister)=>{const editable=isAdmin&&"person_id" in minister&&Boolean(minister.person_id);return <AdminSearchCard admin={editable} linksOnly href={minister.href} label={`${minister.name} 목회자`} className="pastor-home-card shared-pastor-card" key={`${minister.href}-${minister.name}-${minister.role_title}`} controls={editable&&"person_id" in minister?<PastorControls id={minister.person_id} roleId={minister.role_id} name={minister.name} roleTitle={minister.role_title} churchName={church.name} region={church.region} denomination={church.denomination} roleStatus={minister.role_status} status="approved"/>:undefined}><PastorCardContent name={minister.name.replace(/\s*목사(?:님)?$/u,"")} href={minister.href} photoUrl="/pastor-silhouette-soft.png" roleStatus={minister.role_status} roles={[minister.role_title]} churchName={church.name} churchHref={`/church/${church.id}`} region={church.region} denomination={church.denomination} sourceUrl={minister.source_url}/></AdminSearchCard>})}</div></section>}
