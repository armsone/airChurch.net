...
type PersonMinistryRow={person_id:number;public_id:number;role_id:number;name:string;role_title:string;role_category:string;role_status:string;source_url:string|null};
...
export default async function ChurchPage({params}:{params:Promise<{id:string}>}){
  ...
  const [sermons,praises,related,profile,schedules,encouragements,ministries,personMinistries]=await Promise.all([
    db.prepare("SELECT youtube_id,title,published_at FROM sermons WHERE church_id=? AND status='published' ORDER BY published_at DESC LIMIT 9").bind(churchId).all<VideoRow>(),
    db.prepare("SELECT youtube_id,title,published_at FROM praise_videos WHERE church_id=? AND status='published' ORDER BY published_at DESC LIMIT 6").bind(churchId).all<VideoRow>(),
