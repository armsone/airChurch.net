...
type RelatedChurch={id:number;name:string;pastor:string;region:string;denomination:string;priority_weight:number};
...
export default async function ChurchPage({params}:{params:Promise<{id:string}>}){
  ...
  const [sermons,praises,related,profile,schedules,encouragements,ministries,personMinistries]=await Promise.all([
    ...
    db.prepare("SELECT COALESCE(public_id,1000000+id) AS id,name,pastor,region,denomination,priority_weight FROM churches WHERE review_status='approved' AND id!=? AND (region LIKE ? OR denomination=?) ORDER BY RANDOM() LIMIT 6").bind(churchId,regionPrefix,church.denomination).all<RelatedChurch>(),
  ...
  return <main className="church-detail-shell"><SkipLink/>
    ...
    <section className="church-detail-hero" id="primary-content" tabIndex={-1}><div className="church-detail-identity">{image?<img src={image} alt="" width={96} height={96} loading="eager" decoding="async" referrerPolicy="no-referrer"/>:<span aria-hidden="true">교회</span>}<div><small>확인된 공식 정보</small><h1>{church.name}</h1><p><a className="church-pastor-profile-link" href={primaryPerson?`/pastors/${primaryPerson.public_id}`:`/church/${church.public_id}`}>{church.pastor} 목회 기록 보기 →</a> · {church.region} · {church.denomination}</p></div></div><div className="church-detail-actions"><ChurchSaveButton id={church.public_id} name={church.name} pastor={church.pastor} region={church.region}/><ChurchShareButton name={church.name}/>{homepage&&<a href={homepage} target="_blank" rel="noopener noreferrer">공식 홈페이지 ↗</a>}{church.youtube_channel_id&&<a href={`https://www.youtube.com/channel/${church.youtube_channel_id}`} target="_blank" rel="noopener noreferrer">공식 YouTube ↗</a>}<EncouragementJumpLink/></div></section>
