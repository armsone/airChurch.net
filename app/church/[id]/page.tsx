...
type WorshipScheduleRow={record_id:string;service_type:string;day_of_week:string;start_time:string;venue_audience:string|null;source_url:string;collected_at:string|null;reviewed_at:string|null};
...
export default async function ChurchPage({params}:{params:Promise<{id:string}>}){
  ...
  const [sermons,praises,related,profile,schedules,encouragements,ministries,personMinistries]=await Promise.all([
    ...
    db.prepare("SELECT record_id,service_type,day_of_week,start_time,venue_audience,source_url,collected_at,reviewed_at FROM worship_schedules WHERE church_id=? AND review_status='approved' ORDER BY CASE WHEN instr(day_of_week,'SUN')>0 THEN 0 ELSE 1 END,day_of_week,start_time,service_type LIMIT 40").bind(churchId).all<WorshipScheduleRow>(),
