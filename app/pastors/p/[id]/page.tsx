...
export default async function LegacyPastorPersonPage({params}:{params:Promise<{id:string}>}){
  ...
  const person=await db.prepare("SELECT public_id FROM pastor_people WHERE id=? AND review_status='approved' LIMIT 1").bind(internalId).first<{public_id:number|null}>();
