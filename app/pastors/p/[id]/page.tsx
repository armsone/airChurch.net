...
type Person={id:number;name:string;public_summary:string|null;photo_url:string|null;photo_source_url:string|null;updated_at:string};
...
export default async function PastorPersonPage({params}:{params:Promise<{id:string}>}){
  ...
  const primaryRole=roles.results.find((role)=>role.role_status==="current")??roles.results[0];
  ...
  return <main className="church-detail-shell pastor-profile-shell"><SkipLink/><header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink><nav><a href="/#pastor-directory">목회자 찾기</a><a href="/#church-directory">교회 찾기</a><SavedNavLink/><a href="/about">운영 안내</a></nav><a className="church-detail-back" href="/#pastor-directory">목록으로</a></header>
    <section className={`pastor-profile-hero${photo?" has-photo":""}`} id="primary-content" tabIndex={-1}><div className="pastor-profile-identity">{photo&&<figure className={photo.includes("main_visual_")?"is-wide-source":undefined}><img src={`/api/pastor-photo/${person.id}`} alt={`${person.name} 목회자 공식 사진`} width={180} height={220} loading="eager" decoding="async"/>{photoSource&&<figcaption><a href={photoSource} target="_blank" rel="noopener noreferrer">공식 사진 출처 ↗</a></figcaption>}</figure>}<div><span>PASTOR STORY</span><p>공개 출처로 정리한 목회 여정</p><h1>{person.name}<small>{primaryRole?.role_title??"목회자"}</small></h1>{person.public_summary&&<p>{person.public_summary}</p>}</div></div><div className="pastor-profile-actions"><PastorSaveButton personId={person.id} name={person.name} roleTitle={primaryRole?.role_title??"목회자"} churchName={primaryRole?.church_name}/><EncouragementJumpLink/></div></section>
    ...
    {session&&<PastorPrivateContactList items={privateContacts} viewer={session.role==="admin"?"관리자":"승인된 목회자"}/>}<PastorEncouragementBoard pastorId={person.id} title={`${person.name} ${primaryRole?.role_title??"목회자"}님 응원하기`} initialItems={messages.results}/><footer className="church-detail-footer"><a href="/#pastor-directory">목회자 찾기</a><span>공개 출처가 확인되는 정보부터 차분히 채웁니다.</span></footer><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(jsonLd).replace(/</g,"\\u003c")}}/>
  </main>;
}
