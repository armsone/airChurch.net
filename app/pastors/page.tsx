...
type PastorRow={person_id:number|null;public_id:number|null;church_id:number|null;minister_id:number|null;name:string;role_title:string;role_titles:string;role_status:string;church_name:string|null;region:string|null;denomination:string|null;photo_url:string|null;merged_count:number;total_count:number};
...
export default async function PastorsPage({searchParams}:{searchParams:Promise<{q?:string;page?:string}>}){
  ...
  `).all<PastorRow>():await db.prepare(`
    WITH raw_people AS (
      SELECT p.id AS person_id,COALESCE(p.public_id,1000000+p.id) AS public_id,r.church_id,NULL AS minister_id,p.name,r.role_title,r.role_status,r.church_name,r.region,r.denomination,CASE WHEN p.photo_review_status='approved' THEN p.photo_url ELSE NULL END AS photo_url
      ...
      UNION ALL
      SELECT NULL AS person_id,NULL AS public_id,c.id AS church_id,NULL AS minister_id,c.pastor AS name,'담임목사' AS role_title,'current' AS role_status,c.name AS church_name,c.region,c.denomination,NULL AS photo_url
      FROM churches c
      ...
      UNION ALL
      SELECT NULL AS person_id,NULL AS public_id,c.id AS church_id,m.id AS minister_id,m.name,m.role_title,m.role_status,c.name AS church_name,c.region,c.denomination,NULL AS photo_url
  ...
  return <main className="pastor-directory-shell"><SkipLink/><header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink><a className="church-detail-back" href="/portal#pastor-directory">포털로</a></header>
