...
export default async function PastorsPage({searchParams}:{searchParams:Promise<{q?:string;page?:string}>}){
  ...
  return <main className="pastor-directory-shell"><SkipLink/><header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink></header>
    ...
    <section className="pastor-directory-content"><div className="section-heading"><div><span className="section-kicker">PEOPLE</span><h2>{query?`‘${query}’ 검색 결과`:"목회자 목록"}</h2></div><span className="result-count">{total.toLocaleString("ko-KR")}명 · {page}/{totalPages}쪽</span></div><div className="pastor-directory-grid">{unique.length?unique.map((pastor)=>{const href=pastor.person_id&&pastor.public_id!==null?`/pastors/${pastor.public_id}`:`/church/${pastor.church_id}`,photo=pastor.photo_url&&pastor.public_id!==null?`/api/pastor-photo/${pastor.public_id}`:"/pastor-silhouette-soft.png",roles=pastor.role_titles.split(",").filter(Boolean),name=pastor.name.replace(/\s*목사(?:님)?$/u,""),manageable=Boolean(isAdmin&&pastor.person_id),editor=manageable&&pastor.person_id?{id:pastor.person_id,roleId:pastor.role_id,name:pastor.name,roleTitle:pastor.role_title,churchName:pastor.church_name,region:pastor.region,denomination:pastor.denomination,roleStatus:pastor.role_status,status:"approved"}:undefined;return <PastorDirectoryCard key={pastor.person_id?`person-${pastor.person_id}`:`legacy-${pastor.church_id}-${pastor.minister_id??"primary"}`} name={name} href={href} photoUrl={photo} hasPhoto={Boolean(pastor.photo_url)} roleStatus={pastor.role_status} roles={roles} churchName={pastor.church_name} churchHref={pastor.church_id?`/church/${pastor.church_id}`:null} region={pastor.region} denomination={pastor.denomination} detail={pastor.merged_count>1?<i>한 사람의 사역 기록 {pastor.merged_count}건</i>:undefined} admin={manageable} editor={editor}/> }):<p className="empty">찾은 목회자가 없습니다. 이름이나 교회명을 짧게 다시 입력해 보세요.</p>}</div>{unique.length>0&&<nav className="search-pagination" aria-label="목회자 목록 페이지">{page>1?<a href={pageUrl(page-1)}>← 이전 60명</a>:<span/>}<span>{page} / {totalPages}</span>{page<totalPages&&<a href={pageUrl(page+1)}>다음 60명 →</a>}</nav>}</section>
  </main>;
}
