...
import {PastorControls} from "../admin/admin-controls";
...
export default async function PastorsPage({searchParams}:{searchParams:Promise<{q?:string;page?:string}>}){
  ...
  `).all<PastorRow>():await db.prepare(`
    WITH raw_people AS (
      SELECT p.id AS person_id,COALESCE(p.public_id,1000000+p.id) AS public_id,r.id AS role_id,r.church_id,NULL AS minister_id,p.name,r.role_title,r.role_status,r.church_name,r.region,r.denomination,CASE WHEN p.photo_review_status='approved' THEN p.photo_url ELSE NULL END AS photo_url
      ...
      UNION ALL
      SELECT NULL AS person_id,NULL AS public_id,NULL AS role_id,c.id AS church_id,NULL AS minister_id,c.pastor AS name,'담임목사' AS role_title,'current' AS role_status,c.name AS church_name,c.region,c.denomination,NULL AS photo_url
      FROM churches c
      ...
      UNION ALL
      SELECT NULL AS person_id,NULL AS public_id,NULL AS role_id,c.id AS church_id,m.id AS minister_id,m.name,m.role_title,m.role_status,c.name AS church_name,c.region,c.denomination,NULL AS photo_url
  ...
  return <main className="pastor-directory-shell"><SkipLink/><header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink></header>
    <section className="pastor-directory-hero" id="primary-content" tabIndex={-1}><span>PASTOR DIRECTORY</span><h1>목회자를 찾고 응원하세요</h1><p>담임·부교역자·협동·원로·은퇴 목회자의 사역 이력을 한 사람의 기록으로 이어갑니다.</p><form role="search"><label className="sr-only" htmlFor="pastor-query">목회자 검색</label><input id="pastor-query" name="q" defaultValue={query} placeholder="목회자, 교회, 지역, 교단으로 검색"/><button type="submit">찾기</button></form></section>
    <section className="pastor-directory-content"><div className="section-heading"><div><span className="section-kicker">PEOPLE</span><h2>{query?`‘${query}’ 검색 결과`:"목회자 목록"}</h2></div><span className="result-count">{total.toLocaleString("ko-KR")}명 · {page}/{totalPages}쪽</span></div><div className="pastor-directory-grid">{unique.length?unique.map((pastor)=>{const href=pastor.person_id&&pastor.public_id!==null?`/pastors/${pastor.public_id}`:`/church/${pastor.church_id}`,photo=pastor.photo_url&&pastor.public_id!==null?`/api/pastor-photo/${pastor.public_id}`:"/pastor-silhouette-soft.png",roles=pastor.role_titles.split(",").filter(Boolean),name=pastor.name.replace(/\s*목사(?:님)?$/u,""),manageable=Boolean(isAdmin&&pastor.person_id);return <AdminSearchCard admin={manageable} linksOnly href={href} label={`${name} 목회자`} className="pastor-home-card shared-pastor-card" key={pastor.person_id?`person-${pastor.person_id}`:`legacy-${pastor.church_id}-${pastor.minister_id??"primary"}`} controls={manageable&&pastor.person_id?<PastorControls id={pastor.person_id} roleId={pastor.role_id} name={pastor.name} roleTitle={pastor.role_title} churchName={pastor.church_name} region={pastor.region} denomination={pastor.denomination} roleStatus={pastor.role_status} status="approved"/>:undefined}><PastorCardContent name={name} href={href} photoUrl={photo} hasPhoto={Boolean(pastor.photo_url)} roleStatus={pastor.role_status} roles={roles} churchName={pastor.church_name} churchHref={pastor.church_id?`/church/${pastor.church_id}`:null} region={pastor.region} denomination={pastor.denomination} detail={pastor.merged_count>1?<i>한 사람의 사역 기록 {pastor.merged_count}건</i>:undefined}/></AdminSearchCard>}):<p className="empty">찾은 목회자가 없습니다. 이름이나 교회명을 짧게 다시 입력해 보세요.</p>}</div>{unique.length>0&&<nav className="search-pagination" aria-label="목회자 목록 페이지">{page>1?<a href={pageUrl(page-1)}>← 이전 60명</a>:<span/>}<span>{page} / {totalPages}</span>{page<totalPages&&<a href={pageUrl(page+1)}>다음 60명 →</a>}</nav>}</section>
  </main>;
}
