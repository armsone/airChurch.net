...
export default async function PastorsPage({searchParams}:{searchParams:Promise<{q?:string;page?:string}>}){
  ...
  `).all<PastorRow>():await db.prepare(`
    WITH raw_people AS (
      ...
      SELECT REPLACE(name,' ','') AS name_key,COALESCE(church_id,-1) AS church_key,REPLACE(COALESCE(church_name,''),' ','') AS church_name_key,GROUP_CONCAT(DISTINCT role_title) AS role_titles
