...
export default async function PastorProfilePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{minister?:string}>}){
  ...
  return <main className="church-detail-shell pastor-profile-shell"><SkipLink/>
    ...
    <section className="pastor-profile-hero" id="primary-content" tabIndex={-1}><div><span>PASTOR STORY</span><p>공식 공개 정보로 보는 목회 여정</p><h1>{displayName}<small>{roleTitle}</small></h1><a href={`/church/${church.id}`}>{church.name} · {church.region} · {church.denomination}</a><nav className="pastor-profile-sources" aria-label="공식 출처">{ministerSource&&<a href={ministerSource} target="_blank" rel="noopener noreferrer">교역자 공식 출처 ↗</a>}{homepage&&<a href={homepage} target="_blank" rel="noopener noreferrer">교회 홈페이지 ↗</a>}{!minister&&church.youtube_channel_id&&<a href={`https://www.youtube.com/channel/${church.youtube_channel_id}`} target="_blank" rel="noopener noreferrer">공식 YouTube ↗</a>}<EncouragementJumpLink/></nav><PastorSaveButton churchId={church.id} ministerId={minister?.id} name={displayName} roleTitle={roleTitle} churchName={church.name}/></div>{Number(history?.total??0)>0&&<aside><span>airChurch 기록</span><strong>{Number(history?.total??0).toLocaleString("ko-KR")}편</strong><small>연결된 공개 말씀</small></aside>}</section>
