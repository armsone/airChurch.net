...
export default async function PastorProfilePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{minister?:string}>}){
  ...
  if(!church)return <main className="church-detail-shell"><SkipLink/><header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink><a href="/portal#church-directory">교회 찾기</a></header><section className="church-detail-missing" id="primary-content" tabIndex={-1}><span>PASTOR STORY</span><h1>현재 공개된 목회 기록이 아닙니다</h1><p>연결된 교회 정보가 변경되었거나 검토 중일 수 있습니다.</p><a href="/portal#church-directory">다른 교회 찾아보기 →</a></section></main>;
  const db=database();
  ...
  return <main className="church-detail-shell pastor-profile-shell"><SkipLink/>
    <header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink><nav><a href="/portal#sermons">말씀</a><a href="/portal#church-directory">교회 찾기</a><SavedNavLink/><a href="/about">운영 안내</a></nav><a className="church-detail-back" href={`/church/${church.id}`}>교회로</a></header>
    <section className="pastor-profile-hero" id="primary-content" tabIndex={-1}><div><span>PASTOR STORY</span><p>공식 공개 정보로 보는 목회 여정</p><h1>{displayName}<small>{roleTitle}</small></h1><a href={`/church/${church.id}`}>{church.name} · {church.region} · {church.denomination}</a><nav className="pastor-profile-sources" aria-label="공식 출처">{ministerSource&&<a href={ministerSource} target="_blank" rel="noopener noreferrer">교역자 공식 출처 ↗</a>}{homepage&&<a href={homepage} target="_blank" rel="noopener noreferrer">교회 홈페이지 ↗</a>}{!minister&&church.youtube_channel_id&&<a href={`https://www.youtube.com/channel/${church.youtube_channel_id}`} target="_blank" rel="noopener noreferrer">공식 YouTube ↗</a>}</nav>{Number(history?.total??0)>0&&<p className="pastor-profile-record-count">연결된 공개 말씀 {Number(history?.total??0).toLocaleString("ko-KR")}편</p>}</div><div className="pastor-profile-actions"><PastorSaveButton churchId={church.id} ministerId={minister?.id} name={displayName} roleTitle={roleTitle} churchName={church.name}/><EncouragementJumpLink/></div></section>
