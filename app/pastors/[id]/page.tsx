...
import { database, ensureEncouragementTables, ensureSermonTables } from "../../api/_shared";
...
export default async function PastorProfilePage({params}:{params:Promise<{id:string}>}){
  ...
  const db=database();
  await ensureEncouragementTables(db);
  const [history,sermons,encouragements]=await Promise.all([
    ...
    db.prepare("SELECT id,nickname,content,created_at AS createdAt FROM encouragement_messages WHERE church_id=? AND target_type='pastor' AND status='approved' ORDER BY created_at DESC,id DESC LIMIT 30").bind(id).all<EncouragementItem>(),
  ]);
  ...
  return <main className="church-detail-shell pastor-profile-shell"><SkipLink/>
    ...
    <EncouragementBoard churchId={id} targetType="pastor" title={`${church.pastor} 목사님 응원하기`} initialItems={encouragements.results}/>
