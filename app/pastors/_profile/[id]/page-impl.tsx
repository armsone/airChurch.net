...
import {LogoPaletteSection} from "../../../church/[id]/church-detail-hero";
...
type Role={id:number;church_id:number|null;church_public_id:number|null;church_name:string|null;denomination:string|null;region:string|null;role_title:string;role_category:string;role_status:string;start_date:string|null;end_date:string|null;source_url:string|null;church_pastor:string|null;church_priority_weight:number|null};
...
export default async function PastorPersonPage({params}:{params:Promise<{id:string}>}){
  ...
  const db=database(),session=await accessSession();await ensureSermonTables(db);const [roles,messages]=await Promise.all([
    db.prepare("WITH RECURSIVE linked(id) AS (SELECT ? UNION SELECT CASE WHEN i.left_pastor_id=linked.id THEN i.right_pastor_id ELSE i.left_pastor_id END FROM pastor_identity_candidates i JOIN linked ON (i.left_pastor_id=linked.id OR i.right_pastor_id=linked.id) WHERE i.status='confirmed_same') SELECT r.id,r.church_id,CASE WHEN c.id IS NULL THEN NULL ELSE COALESCE(c.public_id,1000000+c.id) END AS church_public_id,r.church_name,r.denomination,r.region,r.role_title,r.role_category,r.role_status,r.start_date,r.end_date,r.source_url,c.pastor AS church_pastor,c.priority_weight AS church_priority_weight FROM pastor_church_roles r LEFT JOIN churches c ON c.id=r.church_id WHERE r.pastor_id IN (SELECT id FROM linked) AND r.review_status='approved' ORDER BY CASE r.role_status WHEN 'current' THEN 0 ELSE 1 END,COALESCE(r.end_date,r.start_date,'') DESC,r.id DESC").bind(person.id).all<Role>(),
  ...
  const paletteImage=safeHttpUrl(churchImageUrls[primaryRole?.church_name??""]||denominationImageUrls[primaryRole?.denomination??""]);
  ...
  return <main className="church-detail-shell pastor-profile-shell"><SkipLink/><header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink></header>
    <LogoPaletteSection image={paletteImage} className={`pastor-profile-hero${photo?" has-photo":""}`}><div className="pastor-profile-identity">{photo&&<figure className={photo.includes("main_visual_")?"is-wide-source":undefined}><img src={`/api/pastor-photo/${person.public_id}`} alt={`${person.name} 목회자 공식 사진`} width={180} height={220} loading="eager" decoding="async"/>{photoSource&&<figcaption><a href={photoSource} target="_blank" rel="noopener noreferrer">공식 사진 출처 ↗</a></figcaption>}</figure>}<div><span>PASTOR STORY</span><p>공개 출처로 정리한 목회 여정</p><h1>{person.name}<small>{displayRoleTitle(primaryRole?.role_title??"목사")}</small></h1>{person.public_summary&&<p>{person.public_summary}</p>}</div></div><div className="pastor-profile-actions"><PastorSaveButton personId={person.id} publicId={person.public_id} name={person.name} roleTitle={displayRoleTitle(primaryRole?.role_title??"목사")} churchName={primaryRole?.church_name}/><EncouragementJumpLink/></div></LogoPaletteSection>
    ...
    <section className="church-detail-content"><div className="section-heading"><div><span className="section-kicker">MINISTRY HISTORY</span><h2>교회와 사역 이력</h2></div><span className="result-count">{roles.results.length}건</span></div><div className="pastor-history-grid">{roles.results.length?roles.results.map((role)=>{const source=safeHttpUrl(role.source_url),manageable=Boolean(isAdmin&&role.church_public_id&&role.church_pastor);return <AdminSearchCard admin={manageable} linksOnly href={role.church_public_id?`/church/${role.church_public_id}`:"#"} label={role.church_name??"소속 교회"} className="shared-church-card" key={role.id} controls={manageable&&role.church_public_id&&role.church_pastor?<ChurchControls id={role.church_public_id} name={role.church_name??"소속 교회 확인 중"} pastor={role.church_pastor} region={role.region??"지역 확인 중"} denomination={role.denomination??"교단 확인 중"} status="approved" holdReason={null} holdNote={null} heldAt={null} priorityWeight={role.church_priority_weight??1}/>:undefined}><ChurchCardContent id={role.church_public_id} name={role.church_name??"소속 교회 확인 중"} pastor={`${person.name} · ${role.role_title}`} pastorHref={`/pastors/${person.public_id}`} region={role.region??(role.role_status==="current"?"현재 사역":"사역 이력")} denomination={role.denomination??"교단 확인 중"} detail={<>{(role.start_date||role.end_date)&&<small>{role.start_date??"시작일 미상"} – {role.end_date??"현재"}</small>}{source&&<a className="pastor-source-link" href={source} target="_blank" rel="noopener noreferrer">공식 출처 확인 ↗</a>}</>}/></AdminSearchCard>}):<p className="empty">확인된 사역 이력을 정리하고 있습니다.</p>}</div></section>
