...
export async function generateMetadata({params}:{params:Promise<{id:string}>}):Promise<Metadata>{
  ...
  const roleTitle=displayRoleTitle(person.role_title??"목사"),title=`${person.name} ${roleTitle} | airChurch`,description=`${person.name} ${roleTitle}의 공개 출처 기반 사역 이력과 응원글입니다.`,images=safeHttpUrl(person.photo_url)?[`/api/pastor-photo/${person.public_id}`]:[];
...
export default async function PastorPersonPage({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="church-detail-shell pastor-profile-shell"><SkipLink/><header className="church-detail-header"><HomeReloadLink className="brand"><span className="brand-mark" aria-hidden="true"/><span>airchurch</span></HomeReloadLink></header>
    ...
    <section className="church-detail-content"><div className="section-heading"><div><span className="section-kicker">MINISTRY HISTORY</span><h2>교회와 사역 이력</h2></div><span className="result-count">{roles.results.length}건</span></div><div className="pastor-history-grid">{roles.results.length?roles.results.map((role)=>{const source=safeHttpUrl(role.source_url);return <article className="shared-church-card" key={role.id}><ChurchCardContent id={role.church_id} name={role.church_name??"소속 교회 확인 중"} pastor={`${person.name} · ${role.role_title}`} pastorHref={`/pastors/${person.public_id}`} region={role.region??(role.role_status==="current"?"현재 사역":"사역 이력")} denomination={role.denomination??"교단 확인 중"} detail={<>{(role.start_date||role.end_date)&&<small>{role.start_date??"시작일 미상"} – {role.end_date??"현재"}</small>}{source&&<a className="pastor-source-link" href={source} target="_blank" rel="noopener noreferrer">공식 출처 확인 ↗</a>}</>}/></article>}):<p className="empty">확인된 사역 이력을 정리하고 있습니다.</p>}</div></section>
