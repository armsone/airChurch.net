...
export default function CcmPlayer({visible,interrupted,onPlay}:{visible:boolean;interrupted:boolean;onPlay:()=>void}) {
  ...
  useEffect(()=>{
    if(!playing||!selected||!frame.current)return;
    let cancelled=false;
  ...
  async function share(song:boolean) {
    ...
    if(song&&current)url.searchParams.set("ccm",current.id);
    url.hash="praises";
    try {
      if(navigator.share)await navigator.share({title:song&&current?current.title:"에어처치 · CCM 듣기",url:url.href});
      else {await navigator.clipboard.writeText(url.href);setNotice(song?"이 찬양의 링크를 복사했어요.":"CCM 듣기 링크를 복사했어요.");}
    } catch(error) {
  ...
  return <div className="ccm-listener" hidden={!visible}>
    <div className="ccm-intro"><p>오늘의 마음에, 찬양 한 곡.</p><a href={ppabangUrl} target="_blank" rel="noopener noreferrer" onClick={()=>setPlaying(false)}>빠방에서 더 듣기 ↗</a></div>
    {loading?<div className="ccm-loading" role="status">♫ 찬양을 준비하고 있어요…</div>:error?<div className="ccm-loading" role="alert"><p>{error}</p><button type="button" onClick={()=>setRetry(value=>value+1)}>다시 불러오기</button></div>:current&&<div className="ccm-layout">
      <div className="ccm-main">
        ...
        <div className="ccm-now"><span className="ccm-eyebrow">{playing?"지금 듣는 찬양":"오늘 함께 들을 찬양"}</span><h3>{current.title}</h3><p>{current.channel}</p>
          ...
          <div className="ccm-actions"><button type="button" onClick={()=>void share(true)}>↗ 이 찬양 나누기</button><a href={`https://www.youtube.com/watch?v=${current.id}`} target="_blank" rel="noopener noreferrer" onClick={()=>setPlaying(false)}>YouTube에서 듣기 ↗</a></div>
        </div>
      </div>
      <div className="ccm-queue"><div className="ccm-queue-heading"><h3>함께 듣는 CCM <span>{items.length}</span></h3><button type="button" onClick={()=>void share(false)} aria-label="CCM 듣기 공유">↗ 나누기</button></div><label className="sr-only" htmlFor="ccm-find">CCM 곡과 찬양팀 찾기</label><input id="ccm-find" className="ccm-find" type="search" value={query} onChange={event=>setQuery(event.target.value)} placeholder="곡 제목 · 찬양팀 찾기"/><div className="ccm-tracks" role="group" aria-label="CCM 재생목록">{shown.map(item=><button type="button" key={item.id} className={`ccm-track${item.id===selected?" is-current":""}`} onClick={()=>choose(item.id)} aria-pressed={item.id===selected}><img src={`https://i.ytimg.com/vi/${item.id}/mqdefault.jpg`} alt="" width={96} height={54} loading="lazy"/><span><strong>{item.title}</strong><small>{item.channel}</small></span><em>{item.id===selected?"♪":duration(item.duration)}</em></button>)}{!shown.length&&<p className="ccm-no-results">찾는 곡이 없어요. 다른 제목이나 찬양팀을 입력해 보세요.</p>}</div><p className="ccm-source">빠방 CCM에서 함께 고른 찬양 · 한 곡이 끝나면 다음 곡으로</p></div>
    </div>}
