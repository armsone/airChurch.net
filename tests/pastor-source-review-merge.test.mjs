...
const officialSource=(extra={})=>({type:"official_church",url:"https://church.example/pastor",identity_contribution:["pastor","church","role"],identity_evidence:{pastor:["김한명"],church:["한교회"],role:["담임목사"]},assertions:[],site_policy:{collection_allowed:true,allowed_path_prefixes:["/pastor"],policy_reviewed_at:"2026-09-01",policy_url:"https://church.example/"},...extra});

test("merges reviewed official sources without publishing them",()=>{
  const result=prepared(),task=result.batches[0].tasks[0];task.decision="ready";task.confirmed_role_title="담임목사";task.confirmed_role_status="current";task.reviewed_at="2026-09-01T00:00:00.000Z";task.note="공식 페이지 확인";task.official_sources=[officialSource()];
...
test("rejects immutable changes, pending work, and unsafe source notes",()=>{
  ...
  const unsafe=prepared(),task=unsafe.batches[0].tasks[0];task.decision="ready";task.confirmed_role_title="담임목사";task.confirmed_role_status="current";task.reviewed_at="2026-09-01T00:00:00.000Z";task.note="공식 확인";task.official_sources=[officialSource({note:"010-1234-5678"})];rehash(unsafe.batches[0]);assert.throws(()=>mergePastorSourceReviewBatches(unsafe.summary,unsafe.batches),/sensitive_value/);
});
