...
test("links church directory cards to verified homepages and official YouTube channels", async () => {
  ...
  assert.match(page,/className="church-directory-refresh"/);
  assert.match(page,/다른 교회 보기/);
  assert.doesNotMatch(page,/다른 교회 만나보기/);
...
test("gives the shorts viewer keyboard controls and continuous automatic looping", async () => {
  ...
  assert.match(page,/new youtube\.Player\(playerFrame/);
  ...
  assert.match(page,/youtube-nocookie\.com\/embed\/\$\{activeShort\.youtubeId\}\?autoplay=1&rel=0&playsinline=1&enablejsapi=1&cc_load_policy=0/);
  assert.doesNotMatch(page,/player\?\.destroy\(\)/);
...
test("adds a safe RSS church-news reader and a direct YouTube praise search", async () => {
  ...
  assert.match(newsRoute,/https:\/\/www\.goodnews1\.com\/rss\/allArticle\.xml/);
  assert.match(newsRoute,/https:\/\/www\.cnews\.or\.kr\/rss\/allArticle\.xml/);
  assert.match(newsRoute,/https:\/\/www\.kehcnews\.co\.kr\/rss\/allArticle\.xml/);
  assert.match(newsRoute,/https:\/\/www\.dangdangnews\.com\/rss\/allArticle\.xml/);
  assert.match(newsRoute,/https:\/\/www\.amennews\.com\/rss\/allArticle\.xml/);
  assert.match(newsRoute,/https:\/\/www\.newsm\.com\/rss\/allArticle\.xml/);
  assert.match(newsRoute,/https:\/\/www\.churchr\.or\.kr\/rss\/allArticle\.xml/);
  const sourceCount=(newsRoute.match(/allowedHost:"/g)||[]).length;
  assert.equal(sourceCount,25,`expected 25 sources, found ${sourceCount}`);
  assert.match(newsRoute,/tag\(item,"pubDate"\)\|\|tag\(item,"dc:date"\)/);
  assert.match(newsRoute,/MAX_PER_SOURCE=2/);
  assert.match(newsRoute,/function capPerSource\(items:NewsItem\[\],limit:number\)/);
  assert.match(newsRoute,/if\(count>=limit\) continue;/);
  assert.match(newsRoute,/capPerSource\(\s*settled\.flatMap/);
  assert.match(newsRoute,/\.slice\(0,50\)/);
  ...
  assert.match(newsRoute,/markUrl:"https:\/\/cdn\.newsnjoy\.or\.kr\/image\/logo\/toplogo_/);
  assert.match(newsRoute,/markUrl:"https:\/\/www\.igoodnews\.net\/image\/logo\/toplogo_/);
  assert.match(newsRoute,/markUrl:"https:\/\/cdn\.churchr\.or\.kr\/image\/logo\/toplogo_/);
  const tones=[...newsRoute.matchAll(/tone:"([^"]+)"/g)].map((match)=>match[1]);
  assert.equal(tones.length,25);
  assert.equal(new Set(tones).size,25);
  assert.doesNotMatch(newsRoute,/google\.com\/s2\/favicons/);
