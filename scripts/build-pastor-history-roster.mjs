...
const ROLE_CATEGORIES = {
  ...
  cooperating: ["협동목사"],
  founding: ["설립목사", "창립목사", "개척목사", "초대목사", "초대담임목사"],
  emeritus: ["원로목사"],
  retired: ["은퇴목사"],
};
...
const GENERIC_NAME = /^(?:(?:교육|협동|설립|창립|개척|초대|초대담임|원로|은퇴|담임|위임|대표|부|수석부|행정|목양)\s*)?목사(?:님)?$|^(?:강도사|전임전도사|교육전도사|전도사)(?:님)?$|^(?:미상|없음|공석|청빙중)$/u;
...
function parsePastorLabel(value, explicitRole = null, requireExplicitRole = false) {
  ...
  let name = label;
  if (!roleTitleClaim) {
    const prefix = label.match(/^((?:(?:교육|협동|설립|창립|개척|초대|초대담임|원로|은퇴|담임|위임|대표|부|수석부|행정|목양)\s*목사|강도사|전임전도사|교육전도사|전도사))(?:님)?\s+(.+)$/u);
    const suffix = label.match(/^(.+?)\s+((?:(?:교육|협동|설립|창립|개척|초대|초대담임|원로|은퇴|담임|위임|대표|부|수석부|행정|목양)\s*목사|강도사|전임전도사|교육전도사|전도사))(?:님)?$/u);
