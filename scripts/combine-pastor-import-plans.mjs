...
const identityLinks=[...new Map(plans.flatMap((plan)=>plan.identityLinks??[]).map((item)=>[[item.leftPersonDirectoryId,item.rightPersonDirectoryId,item.evidenceType,item.evidenceValue].join("|"),item])).values()];
const metadata={generatedAt:new Date().toISOString(),sourceFiles:[baselineFile,additionalFile,curatedFile],published:false,databaseWrites:0,automaticApproval:false,privateDataIncluded:false,people:people.length,roles:roles.length,identityLinks:identityLinks.length,photosPrepared:plans.reduce((sum,plan)=>sum+Number(plan.metadata?.photosPrepared??0),0),photosAutomaticallyPublishable:0,homonymRecordsExcluded:plans.reduce((sum,plan)=>sum+Number(plan.metadata?.homonymRecordsExcluded??0),0),identityLinksAutomaticallyMerged:0,copyrightMode:"structured-facts-only; photo-rights-approval-required"};
