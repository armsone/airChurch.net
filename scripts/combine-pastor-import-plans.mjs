...
const photosPrepared=plans.reduce((sum,plan)=>sum+Number(plan.metadata?.photosPrepared??0),0),photosAutomaticallyPublishable=plans.reduce((sum,plan)=>sum+Number(plan.metadata?.photosAutomaticallyPublishable??0),0);
const metadata={generatedAt:new Date().toISOString(),sourceFiles:[baselineFile,additionalFile,curatedFile],published:false,databaseWrites:0,automaticApproval:false,privateDataIncluded:false,people:people.length,roles:roles.length,nameBasedExclusions:0,photosPrepared,photosAutomaticallyPublishable,photoPolicy:"official-public-clergy-profile; publish-then-notice-and-takedown; third-parties-excluded"};
