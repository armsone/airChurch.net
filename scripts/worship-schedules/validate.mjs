...
const expectedReviewIds=[...(bundle.candidates||[]).map((item)=>item.record_id),...(bundle.profiles||[]).map((item)=>item.profile_id)];
const reviewComplete=Boolean(reviewBytes)&&new Set(reviewList.map((item)=>item.record_id)).size===expectedReviewIds.length&&expectedReviewIds.every((id)=>["approve","reject"].includes(reviews[id]?.decision));
...
const output = { metadata: { schema_version: 2, generated_at: new Date().toISOString(), source: inputPath, dry_run: true, automatic_publication: false, reviewComplete, approvalVerified, approvalDigest:reviewBytes?createHash("sha256").update(reviewBytes).digest("hex"):null, privacyScan:{status:"passed",publicContactFields:0,rawCopyrightContentStored:false}, transportReview:{status:hasHttp?"passed_with_warning":"passed",doesNotAffectEligibility:true,eligibilityImpact:"none"} }, ...validated, approved_profiles: validatedProfiles.approved, held_profiles: validatedProfiles.held, profile_errors: validatedProfiles.errors, collection_held: bundle.held || [], collection_errors: bundle.errors || [] };
