...
export function buildMinistryDataReadiness({worshipReport,worshipCollection,approvedWorshipPlan,pastorRoster,pastorSourceReview=null,pastorCollected=null,approvedPastorPlan=null}){
  ...
  const sourceReviewComplete=pastorSourceReview?.metadata?.mode==="official_source_curation_review"&&pastorSourceReview?.metadata?.complete===true&&Number(pastorSourceReview?.metadata?.task_count)===pastorCandidates&&Number(pastorSourceReview?.metadata?.pending_count)===0;
  const pastorCollectionPresent=collectionIsDry&&collectionMode==="approved_church_roster"&&sourceReviewComplete&&collectedSubjects.length===Number(pastorSourceReview?.metadata?.ready_count??-1);
  ...
  const checks={
    ...
    worship_human_review_complete:approvedWorshipPlan?.metadata?.reviewComplete===true,
...
async function main(){
  ...
  const report=buildMinistryDataReadiness({
    ...
    pastorSourceReview:await optionalJson(arg("--pastor-source-review","out/pastor-history/source-review.json")),
