...
export function buildPastorSourceReviewBatches(roster,churchLimit=25){
  ...
  for(const candidate of roster.candidates??[]){
    ...
    const task={subject_id:subjectId,church_id:churchId,pastor_name:text(identity.pastorName,80),church_name:text(identity.churchName,100),denomination:text(identity.denomination,100),region:text(identity.region,100),role_category:text(candidate.roleCategory,40),eligible_role_titles:(candidate.eligibleRoleTitles??[]).map((item)=>text(item,40)).filter(Boolean),official_homepage_url:text(candidate.officialHomepageUrl,500)||null,discovery_queries:(candidate.discoveryQueries??[]).map((item)=>({purpose:text(item.purpose,40),query:text(item.query,180),accepted_sources:(item.acceptedSources??[]).map((source)=>text(source,60))})),decision:"pending",official_sources:[],discovered_roles:[],reviewed_at:"",note:""};
