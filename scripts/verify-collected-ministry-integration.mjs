...
const checks={
  ...
  official_photos_follow_public_role_policy:Number(importPlan.metadata?.photosAutomaticallyPublishable??-1)===photos.length&&photos.every((photo)=>photo.usageBasis==="official_public_clergy_profile"&&photo.publicationPolicy==="publish_then_notice_and_takedown"&&photo.thirdPartyImagePolicy==="single_person_profile_only"),
  import_plan_matches_collection:importPeople.length===people.length&&importRoles.length===roles.length,
  people_and_roles_pending:importPeople.every((person)=>person.reviewStatus==="pending"&&(["pending","approved"].includes(person.photoReviewStatus))&&(person.photoReviewStatus!=="approved"||(person.photoUsageBasis==="official_public_clergy_profile"&&isHttp(person.photoUrl)&&isHttp(person.photoSourceUrl))))&&importRoles.every((role)=>role.reviewStatus==="pending"),
