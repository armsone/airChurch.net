...
const checks={
  ...
  everything_remains_pending:people.every((item)=>item.reviewStatus==="pending"&&item.photoReviewStatus==="pending")&&roles.every((item)=>item.reviewStatus==="pending")&&links.every((item)=>item.status==="pending"),
  ...
  homonyms_not_excluded:combined.metadata?.homonymRecordsExcluded===0,
  identities_not_merged:combined.metadata?.identityLinksAutomaticallyMerged===0,
  low_load_manifest:manifest.batchSize===100&&manifest.people===people.length&&manifest.roles===roles.length&&manifest.identityLinks===links.length&&chunks.length>0&&chunks.every((item)=>item.hashMatches&&item.withinBatch),
};
const failed=Object.entries(checks).filter(([,value])=>!value).map(([key])=>key);console.log(JSON.stringify({ok:failed.length===0,checks,failed,summary:{people:people.length,roles:roles.length,identityLinks:links.length,photos:combined.metadata?.photosPrepared??0,chunks:chunks.length}},null,2));if(failed.length)process.exitCode=1;
