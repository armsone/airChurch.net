...
const checks={
  ...
  people_and_roles_remain_pending:people.every((item)=>item.reviewStatus==="pending"&&(["pending","approved"].includes(item.photoReviewStatus))&&(item.photoReviewStatus!=="approved"||item.photoUsageBasis==="official_public_clergy_profile"))&&roles.every((item)=>item.reviewStatus==="pending"),
  ...
  official_photos_match_policy:Number(combined.metadata?.photosAutomaticallyPublishable??-1)===people.filter((item)=>item.photoReviewStatus==="approved"&&item.photoUsageBasis==="official_public_clergy_profile").length,
