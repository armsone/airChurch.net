...
for(let offset=0;offset<people.length;offset+=batchSize){
  ...
  for(const person of selected){
    lines.push(`INSERT INTO pastor_people (directory_id,name,public_summary,photo_url,photo_source_url,photo_sha256,photo_review_status,review_status) VALUES (${sql(person.directoryId)},${sql(person.name)},NULL,${sql(person.photoUrl)},${sql(person.photoSourceUrl)},${sql(person.photoSha256)},'pending','pending') ON CONFLICT(directory_id) DO UPDATE SET name=excluded.name,photo_url=COALESCE(excluded.photo_url,pastor_people.photo_url),photo_source_url=COALESCE(excluded.photo_source_url,pastor_people.photo_source_url),photo_sha256=COALESCE(excluded.photo_sha256,pastor_people.photo_sha256),updated_at=CURRENT_TIMESTAMP;`);
