...
async function main(){const args=process.argv.slice(2),value=(name,fallback)=>{const index=args.indexOf(name);return index>=0?args[index+1]:fallback;},batchDir=value("--batch-dir","out/pastor-source-review-batches"),output=value("--output","out/gapck-pastor-review-candidates.json"),limit=Math.min(25,Math.max(1,Number(value("--limit","10"))||10)),files=(await readdir(batchDir)).filter((file)=>/^batch-\d+\.json$/.test(file)).sort(),tasks=(await Promise.all(files.map(async(file)=>(JSON.parse(await readFile(path.join(batchDir,file),"utf8"))).tasks??[]))).flat().filter((task)=>task.decision==="pending"&&task.denomination==="대한예수교장로회 합동"&&task.role_category==="current_primary").slice(0,limit);
