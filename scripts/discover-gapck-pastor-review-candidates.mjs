...
async function main(){const args=process.argv.slice(2),value=(name,fallback)=>{const index=args.indexOf(name);return index>=0?args[index+1]:fallback;},values=(name)=>args.flatMap((arg,index)=>arg===name&&args[index+1]?[args[index+1]]:[]),batchDir=value("--batch-dir","out/pastor-source-review-batches"),output=value("--output","out/gapck-pastor-review-candidates.json"),excludeDir=value("--exclude-output-dir",null),limit=Math.min(25,Math.max(1,Number(value("--limit","10"))||10)),files=(await readdir(batchDir)).filter((file)=>/^batch-\d+\.json$/.test(file)).sort(),allTasks=(await Promise.all(files.map(async(file)=>(JSON.parse(await readFile(path.join(batchDir,file),"utf8"))).tasks??[]))).flat(),excludedSubjectIds=new Set(),excludeFiles=values("--exclude-output");
  if(excludeDir)excludeFiles.push(...(await readdir(excludeDir)).filter((file)=>/^gapck-pastor-review-candidates-round\d+\.json$/.test(file)).map((file)=>path.join(excludeDir,file)));
  for(const excludeFile of new Set(excludeFiles)){const prior=JSON.parse(await readFile(excludeFile,"utf8"));for(const item of prior.items??[])excludedSubjectIds.add(item.subject_id);}
  ...
  for(const task of tasks){const churchUrl=`https://gapck.org/api/v1/eORG_USER_HOMEPAGE_CHURCH_LIST?skip=0&limit=100&sort=1&org_nm=${encodeURIComponent(task.church_name)}`,ministerUrl=`https://gapck.org/api/v1/eMBER_USER_HOMEPAGE_LIST?skip=0&limit=100&sort=15&search_type=minister&mber_nm=${encodeURIComponent(task.pastor_name)}`;if(!robots.isAllowed(churchUrl)||!robots.isAllowed(ministerUrl))throw new Error("gapck_robots_disallowed");try{const church=await getJson(churchUrl);await sleep(2500);const minister=await getJson(ministerUrl);items.push({...classifyGapckCandidate(task,church,minister),official_source_urls:[churchUrl,ministerUrl]});}catch(error){items.push({subject_id:task.subject_id,pastor_name:task.pastor_name,church_name:task.church_name,status:"collection_error",error:String(error?.message??error).slice(0,120),requires_human_review:true,automatic_approval:false});}await sleep(2500);}
